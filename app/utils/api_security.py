...
class RateLimiter:
    ...
    def _check_rate_limit_memory(
        ...
        # 古いリクエストを削除
        self.memory_store[key] = [
            req_time for req_time in self.memory_store[key] if req_time > now - window
        ]
        ...
        def decorator(f: Callable) -> Callable:
            ...
            def decorated_function(*args, **kwargs):
                try:
                    ...
                    # レート制限チェック
                    allowed, info = APISecurityDecorator.rate_limiter.is_allowed(
                        key, limit, window
                    )
