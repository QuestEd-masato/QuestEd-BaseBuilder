...
def validate_string_length(
    text: str, min_length: int = 0, max_length: int = 1000
) -> bool:
    """
    文字列長の検証
...
# ValidationError is already defined above at line 16
...
def validate_ranking_params(
    ranking_type: str, scope: str, scope_id: Optional[int] = None, limit: int = 50
) -> dict:
    """
    ...
    if ranking_type not in VALID_TYPES:
        raise ValidationError(
            f"無効なランキングタイプです。有効な値: {', '.join(VALID_TYPES)}", "ranking_type"
        )

    if scope not in VALID_SCOPES:
        raise ValidationError(f"無効なスコープです。有効な値: {', '.join(VALID_SCOPES)}", "scope")

    if not isinstance(limit, int) or limit < 1 or limit > 100:
        raise ValidationError("表示件数は1から100の間で指定してください", "limit")
...
# Note: validate_ranking_type and validate_scope are already defined above
