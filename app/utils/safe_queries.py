...
class SafeAnalyticsQueries:
    ...
    def get_school_performance_stats() -> List[Dict]:
        ...
        query = (
            ...
            .outerjoin(
                ActivityLog,
                and_(User.id == ActivityLog.student_id, User.role == "student"),
            )
            .filter(School.id.isnot(None))
            .group_by(School.id, School.name)
            .order_by(desc("total_activities"))
        )
    ...
    def get_api_usage_statistics(days: int = 30) -> Dict:
        ...
        total_api_calls = (
            chat_usage + theme_generation + evaluation_usage + curriculum_usage
        )

        # コスト計算（概算）
        gpt35_cost = chat_usage * 0.002  # GPT-3.5使用想定
        gpt4_cost = (
            theme_generation + evaluation_usage + curriculum_usage
        ) * 0.06  # GPT-4使用想定
    ...
    def get_user_activity_summary(user_id: int, days: int = 30) -> Dict:
        ...
        activities = ActivityLog.query.filter(
            and_(
                ActivityLog.student_id == user_id, ActivityLog.created_at >= start_date
            )
        ).count()

        # チャット履歴取得
        chats = ChatHistory.query.filter(
            and_(ChatHistory.user_id == user_id, ChatHistory.created_at >= start_date)
        ).count()

        # TODO・Goal取得
        todos = Todo.query.filter(
            and_(Todo.user_id == user_id, Todo.created_at >= start_date)
        ).count()
    ...
    def get_recent_activities(limit: int = 50) -> List[Dict]:
        ...
        query = (
            ...
            .options(joinedload(ActivityLog.student))
        ...
        return [
            {
                "id": activity.id,
                "student_name": activity.student.display_name
                if activity.student
                else "Unknown",
...
class SafeSearchQueries:
    ...
    @staticmethod
    def search_users(
        search_term: str, requester_user_id: int, limit: int = 20
    ) -> List[Dict]:
        ...
        query = User.query.filter(
            or_(User.full_name.contains(search_term), User.email.contains(search_term))
        )
