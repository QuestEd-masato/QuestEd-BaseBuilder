...
class DatabaseSecurity:
    ...
    # 高リスク操作の定義
    HIGH_RISK_OPERATIONS = ["DELETE", "DROP", "ALTER", "TRUNCATE", "UPDATE"]
    ...
    def _log_high_risk_operation(cls, operation: str, statement: str):
        ...
        security_logger.warning(
            f"HIGH_RISK_DB_OPERATION: {operation} | Statement: {statement[:200]}..."
        )
...
class SecureQueryBuilder:
    ...
    def build_safe_select(
        table: str, fields: List[str], conditions: Dict[str, Any]
    ) -> tuple:
        """
        安全なSELECTクエリを構築
    ...
    def build_safe_update(
        table: str, updates: Dict[str, Any], conditions: Dict[str, Any]
    ) -> tuple:
        """
        安全なUPDATEクエリを構築
...
class AccessControl:
    ...
    def check_table_access(
        ...
        if operation.endswith("_own"):
            ...
            return (
                base_operation in allowed_operations or operation in allowed_operations
            )
    ...
    def filter_query_by_access(
        cls, query: str, user_role: str, user_id: Optional[int] = None
    ) -> str:
        """
...
class DatabaseSecurityManager:
    ...
    def check_access(
        self, table_name: str, operation: str, user_role: str, user_id: int = None
    ) -> bool:
        """アクセス権限の確認"""
        return AccessControl.check_table_access(
            table_name, operation, user_role, user_id
        )
