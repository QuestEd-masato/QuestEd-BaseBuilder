...
def get_curriculum_items(curriculum: Curriculum) -> List[Any]:
    ...
    try:
        ...
        logger.error(
            f"カリキュラム項目取得エラー: {str(e)} (curriculum_id={curriculum.id})"
        )
        items = []
...
def _convert_json_to_items(curriculum: Curriculum) -> List[Any]:
    ...
    try:
        json_data = json.loads(curriculum.content) if isinstance(curriculum.content, str) else curriculum.content
        
        # JSON形式から items 形式に変換
        if isinstance(json_data, list):
            for idx, item in enumerate(json_data):
                # JSON項目を標準的な形式に変換
                converted_item = type('Item', (), {
                    'phase': item.get('phase', f'Phase {idx + 1}'),
                    'week': item.get('week', idx + 1),
                    'hours': item.get('hours', 0),
                    'category': item.get('category', ''),
                    'activity': item.get('activity', item.get('title', '')),
                    'teacher_support': item.get('teacher_support', item.get('description', '')),
...
def calculate_curriculum_statistics(curriculum_data: List[Dict[str, Any]]) -> Dict[str, int]:
    """
    カリキュラム統計を計算
