...
class ErrorHandler:
    ...
    @classmethod
    def handle_exception(cls, error: Exception, context: Dict[str, Any] = None) -> Tuple[Dict[str, Any], int]:
        """
        例外を統一的に処理
    ...
    @classmethod
    def _collect_error_info(cls, error: Exception, context: Dict[str, Any]) -> Dict[str, Any]:
        """
        エラー情報を収集
    ...
    @classmethod
    def _log_error(cls, level: str, error_type: str, error_info: Dict[str, Any]) -> None:
        """
        エラーをログに記録
    ...
    @classmethod
    def _handle_security_incident(cls, error: Exception, error_info: Dict[str, Any]) -> None:
        """
        セキュリティインシデントの処理
    ...
    @classmethod
    def _generate_response(cls, error_type: str, error_info: Dict[str, Any]) -> Dict[str, Any]:
        """
        エラーレスポンスを生成
...
def setup_error_handlers(app):
    ...
    def handle_general_exception(error):
        ...
        return render_template('errors/500.html', 
                             error_message=response_data['error']['message']), status_code
    ...
    def handle_internal_error(error):
        ...
        return render_template('errors/500.html',
                             error_message=response_data['error']['message']), 500
...
class RequestContextLogger:
    ...
    def log_request_end(response):
        """リクエスト終了時のログ"""
        if current_app.debug:
            logger.info(f"REQUEST_END: {response.status_code} {request.method} {request.url}")
        return response
