...
from app.models import Class, ClassEnrollment, User
...
def check_student_class_access(student_id: int, class_id: int) -> bool:
    ...
    try:
        ...
        logger.error(
            f"クラスアクセス権限チェックエラー: {str(e)} (student_id={student_id}, class_id={class_id})"
        )
        return False
...
def check_teacher_class_access(teacher_id: int, class_id: int) -> bool:
    ...
    try:
        ...
        if class_obj:
            logger.debug(
                f"教師クラスアクセス権限確認OK: teacher_id={teacher_id}, class_id={class_id}"
            )
            return True
        else:
            logger.warning(
                f"教師クラスアクセス権限なし: teacher_id={teacher_id}, class_id={class_id}"
            )
            return False

    except Exception as e:
        logger.error(
            f"教師クラスアクセス権限チェックエラー: {str(e)} (teacher_id={teacher_id}, class_id={class_id})"
        )
        return False
