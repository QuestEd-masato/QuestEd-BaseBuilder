...
def register_socketio_events():
    ...
    def handle_join_curriculum_sync(data):
        ...
        if current_user.role == "teacher":
            ...
            curriculum = Curriculum.query.filter_by(
                id=curriculum_id, teacher_id=current_user.id
            ).first()
        else:
            # 学生は所属クラスのカリキュラムのみ
            curriculum = (
                Curriculum.query.join(ClassEnrollment)
                .filter(
        ...
        if curriculum:
            ...
            logger.info(
                f"User {current_user.id} joined curriculum sync room: {curriculum_id}"
            )
    ...
    def handle_leave_curriculum_sync(data):
        ...
        logger.info(
            f"User {current_user.id} left curriculum sync room: {curriculum_id}"
        )
...
class RealtimeSyncNotifier:
    ...
    def notify_sync_started(
        cls, curriculum_id: int, teacher_id: int, sync_info: Dict[str, Any]
    ):
        """同期開始通知"""
        if not socketio:
            logger.warning(
                "SocketIO not initialized, cannot send sync_started notification"
            )
            return
        ...
        # カリキュラム同期ルームに通知
        socketio.emit(
            "sync_notification", notification, room=f"curriculum_sync_{curriculum_id}"
        )
    ...
    def notify_sync_progress(
        cls, curriculum_id: int, teacher_id: int, progress_info: Dict[str, Any]
    ):
        """同期進捗通知"""
        if not socketio:
            logger.warning(
                "SocketIO not initialized, cannot send sync_progress notification"
            )
            return
        ...
        logger.debug(
            f"Sync progress notification sent for curriculum {curriculum_id}: {progress_info}"
        )

    @classmethod
    def notify_sync_completed(
        cls, curriculum_id: int, teacher_id: int, result: Dict[str, Any]
    ):
        """同期完了通知"""
        if not socketio:
            logger.warning(
                "SocketIO not initialized, cannot send sync_completed notification"
            )
            return
        ...
        logger.info(
            f"Sync completed notification sent for curriculum {curriculum_id}: {result.get('message', 'No message')}"
        )

    @classmethod
    def notify_sync_conflict(
        cls, curriculum_id: int, teacher_id: int, conflict_info: Dict[str, Any]
    ):
        """同期競合通知"""
        if not socketio:
            logger.warning(
                "SocketIO not initialized, cannot send sync_conflict notification"
            )
            return
        ...
        logger.warning(
            f"Sync conflict notification sent for curriculum {curriculum_id}: {conflict_info}"
        )

    @classmethod
    def _notify_students_unit_updated(
        cls, curriculum_id: int, sync_result: Dict[str, Any]
    ):
        """学生への単元更新通知"""
        try:
            from app.models import ClassEnrollment, Curriculum, CurriculumUnit
            ...
            socketio.emit(
                ...
                student_notification,
                room=f"class_{curriculum.class_id}",
            )

            logger.info(
                f"Unit update notification sent to class {curriculum.class_id} for curriculum {curriculum_id}"
            )

        except Exception as e:
            logger.error(
                f"Error sending student unit update notification: {str(e)}",
                exc_info=True,
            )
