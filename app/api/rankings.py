...
from app.models import Class, ClassEnrollment, Ranking, RankingCache, User, db
...
def get_total_points_ranking():
    """総合ポイントランキング取得API（ダッシュボード用）- RankingService使用"""
    try:
        # RankingServiceをインポート
        ...
        if scope == "class":
            if current_user.role == "student":
                ...
                if teacher_class:
                    scope_id = teacher_class.id

        # RankingServiceを使用してランキング取得
        ranking_data = RankingService.get_ranking(
            ranking_type="total_points", scope=scope, scope_id=scope_id, limit=limit
        )
        ...
        if ranking_data.get("rankings"):
            for rank_info in ranking_data["rankings"]:
                if rank_info["student_id"] == current_user.id:
                    my_rank = rank_info["rank"]
                    my_score = rank_info["score"]
                    break
        ...
        response_data = {
            ...
            "my_score": my_score,
            "scope": scope,
            "last_updated": ranking_data.get("last_updated", datetime.now().isoformat()),
            "api_version": "1.0"
        }
        ...
        return jsonify({"status": "error", "message": "ランキング取得中にエラーが発生しました"}), 500
...
def get_ranking(ranking_type):
    ...
    try:
        ...
        if current_user.role == "student":
            if not class_id:
                ...
                if enrollment:
                    ...
                    return jsonify({"status": "error", "message": "クラスに所属していません"}), 400
        ...
        # キャッシュから取得を試行
        cached_ranking = RankingCache.query.filter_by(cache_key=cache_key).first()
...
def get_student_ranking(student_id):
    ...
    try:
        ...
        if current_user.role == "student" and current_user.id != student_id:
            return (
                jsonify({"status": "error", "message": "他の学生のランキングを表示する権限がありません"}),
                403,
            )
        ...
        if current_user.role == "teacher":
            ...
            if not student_in_class:
                return (
                    jsonify({"status": "error", "message": "この学生のデータにアクセスする権限がありません"}),
                    403,
                )
        ...
        if not student or student.role != "student":
            return jsonify({"status": "error", "message": "指定された学生が見つかりません"}), 404
        ...
        return jsonify(
            {"status": "success", "student": student_info, "rankings": student_rankings}
        )

    except Exception as e:
        logging.error(f"Get student ranking error: {str(e)}")
        return jsonify({"status": "error", "message": "学生ランキング取得中にエラーが発生しました"}), 500
...
def get_ranking_analytics(class_id):
    ...
    try:
        ...
        if current_user.role == "teacher":
            ...
            if not class_obj:
                return (
                    jsonify({"status": "error", "message": "このクラスにアクセスする権限がありません"}),
                    403,
                )
        else:
            ...
            if not class_obj:
                return jsonify({"status": "error", "message": "指定されたクラスが見つかりません"}), 404
        ...
        for ranking_type in ranking_types:
            ...
            if class_ranking:
                ...
                analytics_data[ranking_type] = {
                    ...
                    "bottom_students": class_ranking[-3:]
                    if len(class_ranking) >= 3
                    else [],
                }
            else:
                analytics_data[ranking_type] = {
        ...
        return jsonify({"status": "error", "message": "ランキング分析取得中にエラーが発生しました"}), 500
...
def clear_ranking_cache():
    ...
    try:
        ...
        if current_user.role != "admin":
            return jsonify({"status": "error", "message": "この機能は管理者のみ利用できます"}), 403
        ...
        logging.info(
            f"Ranking cache cleared: type={cache_type}, count={deleted_count}, admin_id={current_user.id}"
        )
        ...
        db.session.rollback()
        return jsonify({"status": "error", "message": "キャッシュクリア中にエラーが発生しました"}), 500
...
def export_ranking_data():
    ...
    try:
        ...
        if current_user.role == "teacher":
            if class_id:
                ...
                if not class_obj:
                    return (
                        jsonify(
                            {"status": "error", "message": "このクラスのデータをエクスポートする権限がありません"}
                        ),
                        403,
                    )
            else:
                ...
                if not teacher_classes:
                    return (
                        jsonify({"status": "error", "message": "エクスポート可能なクラスがありません"}),
                        400,
                    )
        ...
        if format_type == "csv":
            ...
            response.headers[
                "Content-Disposition"
            ] = f'attachment; filename="ranking_{ranking_type}_{period}.csv"'
        ...
        return jsonify({"status": "error", "message": "データエクスポート中にエラーが発生しました"}), 500
...
def get_user_stats():
    ...
    try:
        ...
        return jsonify(
            {"status": "success", "user_role": current_user.role, "stats": stats}
        )
...
def _generate_ranking_data(
    ranking_type, class_id=None, school_id=None, days_back=None, limit=None
):
    """ランキングデータを生成"""
