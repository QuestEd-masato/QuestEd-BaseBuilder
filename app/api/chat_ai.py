...
from app.models import AIRecommendation, ChatHistory, InquiryTheme, User, db
from app.utils.rate_limiting import api_limit, smart_ai_limit
...
def chat():
    ...
    if request.method == "GET":
        ...
        try:
            ...
            return jsonify({"status": "error", "message": "チャット履歴取得中にエラーが発生しました"}), 500
        ...
        try:
            ...
            if theme_id:
                inquiry_theme = InquiryTheme.query.filter_by(
                    id=theme_id, student_id=current_user.id
                ).first()

                if not inquiry_theme:
                    return (
                        jsonify({"status": "error", "message": "指定されたテーマが見つかりません"}),
                        404,
                    )
            ...
            logging.info(
                f"Chat completed: student_id={current_user.id}, theme_id={theme_id}, response_time={response_time:.2f}s"
            )
            ...
            db.session.rollback()
            return jsonify({"status": "error", "message": "AI応答生成中にエラーが発生しました"}), 500
...
def transcribe_speech():
    ...
    try:
        ...
        if "audio" not in request.files:
            return jsonify({"status": "error", "message": "音声ファイルが必要です"}), 400

        audio_file = request.files["audio"]

        if audio_file.filename == "":
            return jsonify({"status": "error", "message": "音声ファイルが選択されていません"}), 400
        ...
        if file_size > 10 * 1024 * 1024:  # 10MB
            return (
                jsonify({"status": "error", "message": "ファイルサイズが大きすぎます（最大10MB）"}),
                400,
            )
        ...
        try:
            ...
            logging.info(
                f"Speech transcribed: student_id={current_user.id}, filename={audio_file.filename}"
            )
            ...
            return jsonify({"status": "error", "message": "音声認識処理中にエラーが発生しました"}), 500

    except Exception as e:
        logging.error(f"Speech transcribe error: {str(e)}")
        return jsonify({"status": "error", "message": "音声ファイル処理中にエラーが発生しました"}), 500
...
def get_recommendations():
    ...
    try:
        ...
        # 最新の推奨事項を取得
        recommendations = (
            query.order_by(AIRecommendation.created_at.desc()).limit(limit).all()
        )
        ...
        return jsonify({"status": "error", "message": "推奨事項取得中にエラーが発生しました"}), 500
...
def submit_recommendation_feedback(recommendation_id):
    ...
    try:
        ...
        feedback_type = data.get(
            "feedback_type"
        )  # 'helpful', 'not_helpful', 'completed', 'dismissed'
        comment = data.get("comment", "")

        if not feedback_type:
            return jsonify({"status": "error", "message": "フィードバックタイプが必要です"}), 400

        # 推奨事項の確認
        recommendation = AIRecommendation.query.filter_by(
            id=recommendation_id, student_id=current_user.id
        ).first()

        if not recommendation:
            return jsonify({"status": "error", "message": "指定された推奨事項が見つかりません"}), 404
        ...
        logging.info(
            f"Recommendation feedback: rec_id={recommendation_id}, type={feedback_type}, student_id={current_user.id}"
        )

        return jsonify({"status": "success", "message": "フィードバックを送信しました"})

    except Exception as e:
        logging.error(f"Submit recommendation feedback error: {str(e)}")
        db.session.rollback()
        return jsonify({"status": "error", "message": "フィードバック送信中にエラーが発生しました"}), 500
...
def recommendation_settings():
    ...
    if request.method == "GET":
        ...
        try:
            ...
            return jsonify({"status": "error", "message": "設定取得中にエラーが発生しました"}), 500
        ...
        try:
            ...
            logging.info(
                f"Recommendation settings updated: student_id={current_user.id}"
            )
            ...
            return jsonify({"status": "error", "message": "設定更新中にエラーが発生しました"}), 500
    ...
    try:
        ...
        for category in categories:
            ...
            category_stats[category] = {
                ...
                "completion_rate": category_completed / category_count
                if category_count > 0
                else 0,
            }
        ...
        overall_completion_rate = (
            completed_recommendations / recent_recommendations
        ...
        return jsonify({"status": "error", "message": "分析データ取得中にエラーが発生しました"}), 500


@chat_ai_bp.route("/ai/status", methods=["GET"])
def get_ai_status():
    """AI機能利用可能性チェックAPI"""
    try:
        # OpenAI クライアントの利用可能性をチェック
        ...
        return jsonify({
            ...
            "message": "AI機能が利用可能です" if ai_available else "AI機能は現在利用できません（APIキーが設定されていません）"
        })
        
    except Exception as e:
        logging.error(f"AI status check error: {str(e)}")
        return jsonify({
            "status": "error", 
            "ai_available": False,
            "message": "AI機能状態確認中にエラーが発生しました"
        }), 500
