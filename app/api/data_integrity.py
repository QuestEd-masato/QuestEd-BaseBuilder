...
from app.models import db, CurriculumUnit, StudentUnitSelection
...
def verify_data_integrity():
    ...
    try:
        ...
        verification_results['checks'].append({
            ...
            'status': 'OK' if (units_check.null_created_by + units_check.null_school_id + units_check.null_subject_id) == 0 else 'NEEDS_FIX'
        })
        ...
        verification_results['checks'].append({
            ...
            'status': 'OK' if (selections_check.null_study_time + selections_check.invalid_status) == 0 else 'NEEDS_FIX'
        })
...
def recalculate_progress():
    ...
    try:
        ...
        if student_id and unit_id:
            # 特定の学生・単元の進捗再計算
            success = UnitItemMappingService.update_unit_selection_progress(student_id, unit_id)
            ...
            for selection in selections:
                try:
                    ...
                    logger.error(f"Progress update error for selection {selection.id}: {str(e)}")
                    error_count += 1
