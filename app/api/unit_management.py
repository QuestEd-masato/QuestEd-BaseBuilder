...
from app.models import (
    db, CurriculumUnit, StudentUnitSelection, UnitItemMapping, 
    ClassLearningSettings, Class, ClassEnrollment
)
...
def update_unit_progress(unit_id):
    ...
    try:
        ...
        logging.info(f"Unit progress updated: student_id={current_user.id}, unit_id={unit_id}, progress={progress_percentage}%")
...
def create_unit_mappings():
    ...
    try:
        ...
        logging.info(f"Unit mappings created: unit_id={unit_id}, count={created_count}, teacher_id={current_user.id}")
...
def get_my_unit_selections():
    ...
    try:
        ...
        if current_user.role != 'student':
            return jsonify({
                'status': 'error',
                'message': 'この機能は学生のみ利用できます'
            }), 403
        
        # フィルタリングパラメータ
        status = request.args.get('status')  # 'completed', 'in_progress', 'not_started'
        include_unit_details = request.args.get('include_details', 'true').lower() == 'true'
        ...
        for selection in selections:
            selection_info = {
                ...
                'started_at': selection.started_at.isoformat() if selection.started_at else None,
                'completed_at': selection.completed_at.isoformat() if selection.completed_at else None,
                'completion_request_date': selection.completion_request_date.isoformat() if selection.completion_request_date else None,
                'last_activity_at': selection.last_activity_at.isoformat() if selection.last_activity_at else None
            }
            ...
            if include_unit_details and selection.unit:
                selection_info['unit'] = {
                    ...
                    'estimated_hours': selection.unit.estimated_hours,
...
def get_completion_history():
    ...
    try:
        ...
        completed_selections = StudentUnitSelection.query.filter_by(
            student_id=current_user.id,
            status='completed'
        ).order_by(StudentUnitSelection.completed_at.desc()).all()
        ...
        for selection in completed_selections:
            if selection.unit:
                history_item = {
                    ...
                    'progress_percentage': selection.progress_percentage,
                    'study_duration_days': (selection.completed_at - selection.started_at).days if selection.started_at else 0,
                    'approval_status': selection.approval_status,
...
def get_pending_approvals():
    ...
    try:
        ...
        if current_user.role != 'teacher':
            return jsonify({
                'status': 'error',
                'message': 'この機能は教師のみ利用できます'
            }), 403
        
        # 教師のクラスの学生の承認待ち申請を取得
        pending_approvals = db.session.query(StudentUnitSelection).join(
            ClassEnrollment, StudentUnitSelection.student_id == ClassEnrollment.student_id
        ).join(
        ...
        for selection in pending_approvals:
            approval_item = {
                ...
                'student_name': selection.student.name if selection.student else 'Unknown',
                'unit_id': selection.unit_id,
                ...
                'study_duration_days': (selection.completion_request_date - selection.started_at).days if selection.started_at else 0
            }
...
def reject_completion(selection_id):
    ...
    try:
        ...
        logging.info(f"Unit completion rejected: selection_id={selection_id}, teacher_id={current_user.id}")
...
def batch_approve_completions():
    ...
    try:
        ...
        if not isinstance(selection_ids, list):
            return jsonify({
                'status': 'error',
                'message': 'selection_idsは配列形式である必要があります'
            }), 400
...
def get_approval_statistics():
    ...
    try:
        ...
        # 教師のクラスの学生の申請統計を取得
        base_query = db.session.query(StudentUnitSelection).join(
            ClassEnrollment, StudentUnitSelection.student_id == ClassEnrollment.student_id
        ).join(
            Class, ClassEnrollment.class_id == Class.id
