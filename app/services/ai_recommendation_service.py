...
class AIRecommendationService:
    ...
    def generate_recommendations(
        ...
        try:
            ...
            # エラー時のフォールバック推薦
            fallback_result = (
                AIRecommendationService._generate_fallback_recommendations(
    ...
    def _generate_unit_recommendations(
        student_id: int, context_data: Dict, max_items: int, session_id: str
    ) -> Dict:
        """
        単元推薦を生成
        ...
        # 開始可能な単元のみフィルタ
        startable_units = [
            unit for unit in available_units if unit.get("can_start", False)
        ]
    ...
    def _generate_review_recommendations(
        ...
        for weakness in weaknesses:
            # 弱点カテゴリに関連する問題を検索
            problems = (
                BasicKnowledgeItem.query.filter(
                    BasicKnowledgeItem.title.contains(weakness.category)
                )
                .limit(max_items // len(weaknesses) if weaknesses else max_items)
                .all()
            )
    ...
    @staticmethod
    def _generate_problem_recommendations(
        student_id: int, context_data: Dict, max_items: int, session_id: str
    ) -> Dict:
        """
        問題推薦を生成
        ...
        # 生徒の学習履歴から適切な難易度を判定
        recent_answers = (
            AnswerRecord.query.filter_by(student_id=student_id)
        ...
        problems = (
            BasicKnowledgeItem.query.filter_by(
                difficulty=target_difficulty, is_active=True
            )
            .limit(max_items)
            .all()
        )
    ...
    def _generate_fallback_recommendations(
        student_id: int, recommendation_type: str, max_items: int
    ) -> List[Dict]:
        """
        フォールバック推薦を生成（AI失敗時）
        ...
        if recommendation_type == "unit":
            # 次に学習すべき単元（順序ベース）
            units = (
                CurriculumUnit.query.filter_by(is_active=True)
            ...
            # 基礎レベルの問題
            problems = (
                BasicKnowledgeItem.query.filter_by(difficulty_level=1, is_active=True)
                .limit(max_items)
                .all()
            )
    ...
    def _collect_context_data(student_id: int) -> Dict:
        ...
        recent_performance = {
            ...
            "accuracy_rate": (
                sum(1 for answer in recent_answers if answer.is_correct)
                / len(recent_answers)
            )
            if recent_answers
            else 0,
    ...
    def provide_feedback(
        recommendation_id: int, is_accepted: bool, feedback_text: str = None
    ) -> Dict:
        """
        推薦に対するフィードバックを記録
    ...
    def update_recommendation_settings(student_id: int, settings_data: Dict) -> Dict:
        ...
        default_settings = AIRecommendationService.get_recommendation_settings(
            student_id
        )
    ...
    def analyze_learning_patterns(student_id: int) -> Dict:
        ...
        # 難易度選好パターンの分析
        difficulty_data = AIRecommendationService._analyze_difficulty_preference(
            student_id
        )
        AIRecommendationService._update_learning_pattern(
    ...
    @staticmethod
    def _update_learning_pattern(
        student_id: int, pattern_type: str, pattern_data: Dict
    ):
        """学習パターンを更新"""
