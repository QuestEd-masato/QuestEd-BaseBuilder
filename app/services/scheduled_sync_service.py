...
class ScheduledSyncService:
    ...
    def run_scheduled_sync_check(cls) -> Dict[str, any]:
        ...
        try:
                ...
                try:
                    ...
                    if should_sync:
                        ...
                        if result['success']:
                            ...
                            logger.info(f"Scheduled sync successful for curriculum {curriculum.id}")
                        elif result.get('requires_user_action'):
                            sync_results['sync_conflicts'] += 1
                            logger.warning(f"Scheduled sync conflict for curriculum {curriculum.id}")
                        else:
                            sync_results['sync_failed'] += 1
                            logger.error(f"Scheduled sync failed for curriculum {curriculum.id}")
                    ...
                    logger.error(f"Scheduled sync error for curriculum {curriculum.id}: {str(e)}", exc_info=True)
                    sync_results['sync_failed'] += 1
                    sync_results['details'].append({
                        'curriculum_id': curriculum.id,
                        'curriculum_title': curriculum.title,
            ...
            return {
                'success': False,
                'message': f'スケジュール同期チェック中にエラーが発生しました: {str(e)}'
            }
    ...
    def _get_auto_sync_enabled_curriculums(cls) -> List[Curriculum]:
        ...
        try:
            ...
            logger.error(f"Get auto sync enabled curriculums error: {str(e)}", exc_info=True)
            return []
    ...
    def get_scheduled_sync_summary(cls) -> Dict[str, any]:
        ...
        try:
            ...
            for curriculum in cls._get_auto_sync_enabled_curriculums():
                ...
                for log in sync_logs:
                    try:
                        ...
                        if started_at > cutoff_time and log.get('trigger_type') == 'scheduled':
                            recent_sync_count += 1
            ...
            return {
                ...
                'next_check': (datetime.utcnow() + timedelta(hours=1)).isoformat()  # 1時間後
            }
