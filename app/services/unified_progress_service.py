...
class UnifiedProgressService:
    ...
    def _get_progress_overview(self, student_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            completed_units = len(
                [s for s in unit_selections if s.status == "completed"]
            )
            ...
            strong_categories = len(
                [cp for cp in category_proficiencies if cp.adjusted_proficiency >= 4]
            )
            ...
            completed_reviews = len(
                [rs for rs in review_sets if rs.status == "completed"]
            )
            ...
            return {
                "self_paced_learning": {
                    ...
                    "completion_rate": (completed_units / total_units * 100)
                    if total_units > 0
                    else 0,
                },
                "vocabulary_learning": {
                ...
                "category_proficiency": {
                    ...
                    "strength_rate": (strong_categories / total_categories * 100)
    ...
    def _get_self_paced_progress(self, student_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            for selection in selections:
                ...
                # 学習時間累計
                progress_data["total_study_time_minutes"] += (
                    selection.study_time_minutes or 0
                )
                ...
                if (
                    selection.unit
                    and selection.unit.difficulty
                    in progress_data["difficulty_distribution"]
                ):
                    progress_data["difficulty_distribution"][
                        selection.unit.difficulty
                    ] += 1
    ...
    def _get_basebuilder_progress(self, student_id: int) -> Dict[str, Any]:
        ...
        try:
            # 単語レベル熟練度
            word_proficiencies = (
                WordProficiency.query.filter_by(student_id=student_id)
            ...
            category_proficiencies = (
                ProficiencyRecord.query.filter_by(student_id=student_id)
            ...
            if len(word_proficiencies) > 0:
                progress_data["word_statistics"][
                    "average_level"
                ] = total_word_level / len(word_proficiencies)
            ...
            if len(category_proficiencies) > 0:
                progress_data["category_statistics"][
                    "average_proficiency"
                ] = total_category_proficiency / len(category_proficiencies)
            ...
            if len(path_assignments) > 0:
                progress_data["learning_paths"][
                    "average_progress"
                ] = total_path_progress / len(path_assignments)
                ...
                try:
                    if wp.problem:
                        progress_data["recent_word_progress"].append(
                            {
                                "word": wp.problem.title
                                if wp.problem.title
                                else "Unknown",
                                "category": wp.problem.category.name
                                if (wp.problem.category)
                                else "Unknown",
                        ...
                        progress_data["recent_category_progress"].append(
                            {
                                ...
                                "adjusted_proficiency": getattr(
                                    cp, "adjusted_proficiency", 0.0
                                ),
        ...
        try:
            ...
            return {
                ...
                "selected_theme": {
                    ...
                    "created_at": selected_theme.created_at.isoformat()
                    if selected_theme and selected_theme.created_at
                    else None,
                }
                if selected_theme
                else None,
    ...
    def _get_ai_recommendation_status(self, student_id: int) -> Dict[str, Any]:
        ...
        try:
            recommendations = (
                AIRecommendation.query.filter_by(student_id=student_id)
            ...
            accepted_recommendations = [
                r for r in recommendations if r.is_accepted is True
            ]
            effective_recommendations = [
                r for r in recommendations if r.is_effective is True
            ]
            ...
            return {
                ...
                "acceptance_rate": (
                    len(accepted_recommendations) / len(recommendations) * 100
                )
                if recommendations
                else 0,
                "effectiveness_rate": (
                    len(effective_recommendations) / len(recommendations) * 100
                )
                if recommendations
                else 0,
                "last_recommendation_date": recommendations[0].created_at.isoformat()
                if recommendations
                else None,
            }
    ...
    def _get_review_system_progress(self, student_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "high_priority_weaknesses": len(
                    [w for w in weaknesses if w.severity_level >= 4]
                ),
            }
        ...
        try:
            ...
            return {
                ...
                "goals": {
                    ...
                    "in_progress": len(
                        [g for g in goals if not g.is_completed and g.progress > 0]
                    ),
    ...
    def _calculate_weekly_study_time(self, student_id: int) -> int:
        ...
        try:
            ...
            # 単元学習時間
            unit_time = (
                db.session.query(func.sum(StudentUnitSelection.study_time_minutes))
                .filter(
                    and_(
                        StudentUnitSelection.student_id == student_id,
    ...
    def _analyze_learning_patterns(self, student_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "preferred_session_length": self._calculate_preferred_session_length(
                    student_id
                ),
            }
    ...
    def _count_recent_activities(
        self, student_id: int, days: int = 7, offset: int = 0
    ) -> int:
        """
        指定期間内の学習活動数を正確にカウント
