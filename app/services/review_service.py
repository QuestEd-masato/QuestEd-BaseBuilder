...
from app.models import ReviewSet, ReviewSetItem, User
...
class ReviewService:
    ...
    def get_review_sets(
        ...
        review_sets = (
            query.order_by(ReviewSet.created_at.desc())
            .offset(offset)
            .limit(limit)
            .all()
        )
    ...
    def get_review_set_detail(set_id: int, student_id: int) -> Optional[Dict]:
        ...
        Returns:
            復習セット詳細
        """
        review_set = ReviewSet.query.filter_by(id=set_id, student_id=student_id).first()
        ...
        # 復習問題詳細を取得
        items_query = (
            db.session.query(ReviewSetItem, BasicKnowledgeItem)
        ...
        performance_summary = {
            ...
            "accuracy_rate": (correct_items / completed_items * 100)
            if completed_items > 0
            else 0.0,
            "average_time_per_problem": total_time / completed_items
            if completed_items > 0
            else 0,
            "total_study_time": total_time,
        }

        set_data.update({"items": items, "performance_summary": performance_summary})
    ...
    def submit_answer(
        ...
        review_item = ReviewSetItem.query.filter_by(
            id=item_id, review_set_id=set_id
        ).first()
        ...
        completed_count = ReviewSetItem.query.filter_by(
            review_set_id=set_id, is_completed=True
        ).count()
    ...
    def complete_review_set(
        ...
        return {
            ...
            "achievements": ReviewService._generate_achievements(
                accuracy_rate, completion_time_minutes
            ),
    ...
    @staticmethod
    def _select_problems_rule_based(
        student_id: int,
        weakness_areas: List[str],
        difficulty_level: int,
        total_problems: int,
        ...
        for area in weakness_areas:
            problems = (
                ...
                .limit(
                    total_problems // len(weakness_areas)
                    if weakness_areas
                    else total_problems
                )
                .all()
    ...
    @staticmethod
    def _update_spaced_repetition_schedule(
        student_id: int, problem_id: int, is_correct: bool
    ):
        """間隔反復学習スケジュールを更新"""
    ...
    def _generate_achievements(
        accuracy_rate: float, completion_time: int = None
    ) -> List[str]:
        """成果バッジを生成"""
        achievements = []
