...
class AutoSyncService:
    ...
    def detect_curriculum_changes(cls, curriculum_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            change_details = []
            if changes_detected:
                change_details = cls._analyze_curriculum_changes(
                    curriculum_id, curriculum_data
                )
            ...
            return {
                ...
                "auto_sync_enabled": curriculum_data.get("auto_sync_settings", {}).get(
    ...
    def _analyze_curriculum_changes(
        cls, curriculum_id: int, curriculum_data: Dict
    ) -> List[Dict]:
        """カリキュラム変更の詳細分析"""
        changes = []
        ...
        try:
            ...
            # アイテムの比較
            current_titles = {
                item.get("activity", ""): idx for idx, item in enumerate(current_items)
            }
            last_titles = {
                item.get("activity", ""): idx for idx, item in enumerate(last_items)
            }
            ...
            for title in current_titles:
                if title in last_titles:
                    ...
                    current_item = (
                        current_items[current_idx]
                        if current_idx < len(current_items)
                        else {}
                    )
                    last_item = (
                        last_items[last_idx] if last_idx < len(last_items) else {}
                    )

                    # 内容の比較
                    if json.dumps(current_item, sort_keys=True) != json.dumps(
                        last_item, sort_keys=True
                    ):
                        changes.append(
                            {
        ...
        try:
            ...
            if last_update > delay_threshold:
                return False, {
                    ...
                    "retry_after": (
                        last_update + timedelta(minutes=sync_delay)
                    ).isoformat(),
                }
    ...
    def execute_auto_sync(
        cls,
        curriculum_id: int,
        trigger_type: SyncTriggerType = SyncTriggerType.AUTO_UPDATE,
    ) -> Dict[str, Any]:
        """自動同期の実行"""
        sync_log_id = None
        try:
            ...
            if conflict_result["has_conflicts"]:
                ...
                return cls._handle_conflicts(
                    curriculum_id, conflict_result, sync_log_id
                )
    ...
    def _check_for_conflicts(cls, curriculum_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "has_conflicts": True,
                "conflicts": [
                    {"type": "error", "description": str(e), "severity": "high"}
                ],
                "error": str(e),
            }
    ...
    def _handle_conflicts(
        cls, curriculum_id: int, conflict_result: Dict, sync_log_id: str
    ) -> Dict[str, Any]:
        """競合の処理"""
        try:
            curriculum = Curriculum.query.get(curriculum_id)
            ...
            resolution_strategy = sync_settings.get(
                "conflict_resolution_strategy", "prompt"
            )

            if resolution_strategy == "auto":
                # 自動解決
                return cls._auto_resolve_conflicts(
                    curriculum_id, conflict_result, sync_log_id
                )
            elif resolution_strategy == "prompt":
                # ユーザーに確認を要求
                cls._complete_sync_log(
                    sync_log_id, SyncStatus.CONFLICT, conflict_result
                )
                return {
                    "success": False,
            ...
            return {"success": False, "message": f"競合処理中にエラーが発生しました: {str(e)}"}
    ...
    def _auto_resolve_conflicts(
        ...
        try:
            ...
            for conflict in conflict_result["conflicts"]:
                if conflict["type"] == "active_learning":
                    ...
                    if (
                        ...
                        and conflict.get("affected_students", 0) < 5
                    ):
                        # 少数の学生の場合は続行
            ...
            return {"success": False, "message": f"自動競合解決中にエラーが発生しました: {str(e)}"}
    ...
    def _create_sync_log(cls, curriculum_id: int, trigger_type: SyncTriggerType) -> str:
        ...
        try:
            ...
            sync_log = {
                ...
                "curriculum_version": curriculum.updated_at.isoformat()
    ...
    def _complete_sync_log(
        cls, sync_log_id: str, status: SyncStatus, result: Dict
    ) -> None:
        """同期ログの完了"""
        try:
            ...
            for log in reversed(sync_logs):
                if log.get("status") == SyncStatus.IN_PROGRESS.value:
                    ...
                    if datetime.utcnow() - started_at > timedelta(minutes=30):
                        ...
                        curriculum.curriculum_data = json.dumps(
                            curriculum_data, ensure_ascii=False
                        )
                        db.session.commit()
                        return False
                    return True
    ...
    def _capture_curriculum_state(cls, curriculum_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "curriculum_updated_at": curriculum.updated_at.isoformat()
                if curriculum.updated_at
                else None,
                "curriculum_data_hash": hashlib.md5(
                    (curriculum.curriculum_data or "").encode("utf-8")
                ).hexdigest(),
    ...
    def _send_sync_notification(
        cls, curriculum_id: int, status: str, result: Dict
    ) -> None:
        ...
        try:
            ...
            logger.info(
                f"Real-time sync notification sent: curriculum_id={curriculum_id}, status={status}"
            )
    ...
    def update_sync_settings(
        cls, curriculum_id: int, settings: Dict[str, Any], user_id: int
    ) -> Dict[str, Any]:
        """同期設定の更新"""
        try:
            curriculum = Curriculum.query.get(curriculum_id)
            ...
            logger.info(
                f"Sync settings updated for curriculum {curriculum_id} by user {user_id}"
            )
            return {
                "success": True,
