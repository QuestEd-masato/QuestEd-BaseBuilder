...
from app.models import CurriculumUnit, StudentUnitSelection, User
from basebuilder.models import AnswerRecord, BasicKnowledgeItem, UnitItemMapping
...
class UnitProgressManager:
    ...
    def update_unit_progress(student_id: int, unit_id: int) -> Dict[str, any]:
        ...
        try:
            ...
            if not related_problems:
                ...
                return {"success": False, "message": "単元に関連する問題が見つかりません", "progress": 0}
    ...
    @staticmethod
    def _calculate_progress_from_records(
        student_id: int, problem_ids: List[int]
    ) -> Dict[str, any]:
        """学習記録から進捗データを計算"""
        try:
            if not problem_ids:
            ...
            correct_problems = len(
                set(record.problem_id for record in records if record.is_correct)
            )
            last_activity = max((record.created_at for record in records), default=None)

            # 進捗率計算（正解した問題の割合）
            progress_percentage = (
                (correct_problems / len(problem_ids)) * 100 if problem_ids else 0
            )
    ...
    def create_unit_item_mappings() -> Dict[str, any]:
        ...
        try:
            ...
            for unit in units:
                # 既存のマッピングをチェック
                existing_count = UnitItemMapping.query.filter_by(
                    unit_id=unit.id
                ).count()
                if existing_count > 0:
                    continue
                ...
                # 関連問題を特定
                related_problems = UnitProgressManager._find_related_problems_for_unit(
                    unit
                )
                ...
                for index, problem in enumerate(related_problems):
                    mapping = UnitItemMapping(
                        ...
                        weight=UnitProgressManager._calculate_relevance_weight(
                            unit, problem
                        ),
                        order_index=index,
                        is_required=True,
            ...
            return {"success": False, "error": str(e), "created_mappings": 0}
    ...
    def batch_update_all_progress() -> Dict[str, any]:
        ...
        try:
            ...
            # アクティブな単元選択を取得
            selections = (
                StudentUnitSelection.query.filter_by(status="not_started")
                .limit(100)
                .all()
            )  # バッチサイズ制限
