...
from app.utils.exceptions import InsufficientDataError, WeaknessAnalysisError
...
class WeaknessAnalyzer:
    ...
    def analyze_student_weaknesses(
        self, student_id: int, force_update: bool = False
    ) -> List[Dict[str, Any]]:
        """
        学生の弱点を総合的に分析
        ...
        try:
            ...
            # 1. 概念理解の弱点分析
            concept_weaknesses = self._analyze_concept_weaknesses(
            ...
            # 3. 知識定着の弱点分析
            knowledge_weaknesses = self._analyze_knowledge_weaknesses(
                student_id, learning_data
            )
            weaknesses.extend(knowledge_weaknesses)

            # 4. 応用力の弱点分析
            application_weaknesses = self._analyze_application_weaknesses(
                student_id, learning_data
            )
            weaknesses.extend(application_weaknesses)

            # 5. 学習パターンの弱点分析
            pattern_weaknesses = self._analyze_learning_pattern_weaknesses(
                student_id, learning_data
            )
            weaknesses.extend(pattern_weaknesses)

            # 6. BaseBuilder特有の弱点分析
            basebuilder_weaknesses = self._analyze_basebuilder_specific_weaknesses(
                student_id, learning_data
            )
    ...
    def _get_recent_analysis(
        self, student_id: int, hours: int = 24
    ...
    def _collect_comprehensive_learning_data(self, student_id: int) -> Dict[str, Any]:
        ...
        try:
                    ...
                    data["proficiency_records"].append(
                        {
                            ...
                            "subject_name": record.category.subject.name
                            if (
                                record.category
                                and hasattr(record.category, "subject")
                                and record.category.subject
                            )
                            else "Unknown",
                    ...
                    data["word_proficiency"].append(
                        {
                            ...
                            "category_id": wp.problem.category_id
                            if wp.problem
                            else None,
                            "category_name": wp.problem.category.name
                            if (wp.problem and wp.problem.category)
                            else "Unknown",
                    ...
                    data["answer_records"].append(
                        {
                            ...
                            "category_id": answer.problem.category_id
                            if answer.problem
                            else None,
                            "category_name": answer.problem.category.name
                            if (answer.problem and answer.problem.category)
                            else "Unknown",
                            "is_correct": getattr(answer, "is_correct", False),
                            "answer_time_seconds": getattr(
                                answer, "answer_time_seconds", 0
                            ),
            ...
            # BaseBuilder学習パス進捗
            path_assignments = (
                PathAssignment.query.filter_by(student_id=student_id)
                .order_by(desc(PathAssignment.assigned_at))
                .all()
            )
            ...
            # 単元選択履歴の収集
            unit_selections = (
                StudentUnitSelection.query.filter_by(student_id=student_id)
                .order_by(desc(StudentUnitSelection.last_activity_at))
                .all()
            )
                    ...
                    data["unit_selections"].append(
                        {
                            ...
                            if selection.progress_percentage
                            else 0,
            ...
            # カテゴリ別パフォーマンス分析
            data["categories_performance"] = self._analyze_category_performance(
                proficiency_records
            )

            # 科目別パフォーマンス分析
            data["subjects_performance"] = self._analyze_subject_performance(
                proficiency_records
            )
    ...
    def _analyze_category_performance(
        ...
        for cat_id, stats in category_stats.items():
            if stats["accuracy_rates"]:
                ...
                stats["accuracy_variance"] = (
                    statistics.variance(stats["accuracy_rates"])
                    if len(stats["accuracy_rates"]) > 1
                    else 0
                )
        ...
        for difficulty, stats in difficulty_stats.items():
            if stats["total_units"] > 0:
                ...
                stats["completion_rate"] = (
                    stats["completed_units"] / stats["total_units"]
                )
    ...
    def _analyze_time_patterns(self, unit_selections: List) -> Dict[str, Any]:
        ...
        if time_patterns["session_lengths"]:
            time_patterns["avg_session_length"] = statistics.mean(
                time_patterns["session_lengths"]
            )
            time_patterns["session_variance"] = (
                statistics.variance(time_patterns["session_lengths"])
                if len(time_patterns["session_lengths"]) > 1
                else 0
            )
    ...
    def _analyze_error_patterns(
        ...
        for record in proficiency_records:
            if (
                record["accuracy_rate"] < self.weak_accuracy_threshold
                and record["total_attempted"] >= self.min_attempts_threshold
            ):
    ...
    def _analyze_basebuilder_performance(
        ...
        try:
            ...
            stats["vocabulary_mastery"] = {
                ...
                "mastered_words": level_distribution[5]
                + level_distribution[4],  # レベル4-5を習得済み
                "struggling_words": level_distribution[0]
                + level_distribution[1],  # レベル0-1は苦手
            }
            ...
            if answer_records:
                # 難易度別パフォーマンス
                difficulty_performance = defaultdict(
                    lambda: {"correct": 0, "total": 0, "avg_time": 0}
                )
                ...
                for record in answer_records:
                    ...
                    if record["answer_time_seconds"]:
                        difficulty_performance[diff]["avg_time"] += record[
                            "answer_time_seconds"
                        ]
            ...
            for wp in word_proficiencies:
                if wp["repetition_number"] and wp["level"]:
                    repetition_effectiveness[wp["repetition_number"]].append(
                        wp["level"]
                    )
    ...
    def _analyze_concept_weaknesses(
        self, student_id: int, learning_data: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        """概念理解の弱点を分析"""
        weaknesses = []
        ...
        for cat_id, stats in categories_performance.items():
            if (
                stats["avg_accuracy"] < self.weak_accuracy_threshold
                and stats["total_attempts"] >= self.min_attempts_threshold
            ):
                severity = self._calculate_severity(
                    stats["avg_accuracy"], stats["total_attempts"]
                )
                confidence = self._calculate_confidence(
                    stats["total_attempts"], stats.get("accuracy_variance", 0)
                )
                ...
                weakness = {
                    ...
                    "recommended_actions": self._generate_concept_recommendations(
                        stats
                    ),
                    "analysis_data": {
                        "category_id": cat_id,
        ...
        for difficulty, stats in difficulty_trends.items():
            if (
                ...
                weakness = {
                    ...
                    "recommended_actions": self._generate_skill_recommendations(
                        difficulty, stats
                    ),
                    "analysis_data": {
    ...
    def _analyze_knowledge_weaknesses(
        self, student_id: int, learning_data: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        """知識定着の弱点を分析"""
        weaknesses = []
        ...
        for subject_name, stats in subjects_performance.items():
            if (
                ...
                weakness = {
                    ...
                    "accuracy_rate": stats["avg_accuracy"],
                    "improvement_trend": "stable",
                    "recommended_actions": self._generate_knowledge_recommendations(
                        subject_name, stats
                    ),
                    "analysis_data": {
    ...
    def _analyze_application_weaknesses(
        self, student_id: int, learning_data: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        """応用力の弱点を分析"""
        weaknesses = []
        ...
        if 3 in difficulty_trends:
            ...
            if stats["total_units"] >= 1 and stats["avg_accuracy"] < 0.6:  # 応用問題は基準を下げる
                ...
                weakness = {
                    ...
                    "accuracy_rate": stats["avg_accuracy"],
                    "improvement_trend": "stable",
                    "recommended_actions": self._generate_application_recommendations(
                        stats
                    ),
                    "analysis_data": {
                        "difficulty_level": 3,
    ...
    def _analyze_learning_pattern_weaknesses(
        self, student_id: int, learning_data: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        """学習パターンの弱点を分析"""
        weaknesses = []
        ...
        # 学習時間の不安定性をチェック
        if (
            "session_variance" in time_patterns
            and time_patterns["session_variance"] > 1000
        ):  # 高い分散
            weakness = {
                "student_id": student_id,
                "category": "学習パターン",
    ...
    def _analyze_basebuilder_specific_weaknesses(
        self, student_id: int, learning_data: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        """BaseBuilder特有の弱点を分析"""
        weaknesses = []
        ...
        if vocabulary_mastery.get("struggling_words", 0) > vocabulary_mastery.get(
            "mastered_words", 0
        ):
            weakness = {
                "student_id": student_id,
                ...
                "accuracy_rate": vocabulary_mastery.get("mastered_words", 0)
                / max(vocabulary_mastery.get("total_words", 1), 1),
                ...
                "analysis_data": {
                    ...
                    "level_distribution": vocabulary_mastery.get(
                        "level_distribution", {}
                    ),
                },
            }
            ...
            if stats.get("is_weak", False) and stats.get("word_count", 0) >= 3:
                ...
                weakness = {
                    ...
                    "correct_attempts": int(
                        stats["average_level"] * stats["word_count"] / 5
                    ),
        ...
        high_dependency_levels = [
            diff
            for diff, usage in hint_dependency.items()
        ...
        if high_dependency_levels:
            weakness = {
                ...
                "confidence_score": 0.7,
                "total_attempts": sum(
                    hint_dependency[diff]["total"] for diff in high_dependency_levels
                ),
                "correct_attempts": 0,
                "accuracy_rate": 0.0,
                "improvement_trend": "stable",
        ...
        poor_retention_reps = [
            rep
            for rep, stats in spaced_repetition.items()
        ...
        if poor_retention_reps:
            weakness = {
                ...
                "total_attempts": sum(
                    spaced_repetition[rep]["sample_size"] for rep in poor_retention_reps
                ),
                "correct_attempts": 0,
                "accuracy_rate": 0.0,
                "improvement_trend": "declining",
    ...
    def _determine_trend(self, stats: Dict[str, Any]) -> str:
        ...
        if len(accuracy_rates) >= 3:
            ...
            earlier_avg = (
                statistics.mean(accuracy_rates[:-3])
                if len(accuracy_rates) > 3
                else recent_avg
            )
    ...
    def _generate_concept_recommendations(self, stats: Dict[str, Any]) -> List[str]:
        ...
        if stats["avg_accuracy"] < 0.5:
            recommendations.extend(
                ["基礎概念の復習を重点的に行う", "より易しい問題から段階的に取り組む", "概念マップや図解を活用した学習"]
            )
        else:
            recommendations.extend(["類似問題での練習を増やす", "異なる角度からの問題に挑戦", "概念の応用例を学習"])

        return recommendations

    def _generate_skill_recommendations(
        self, difficulty: int, stats: Dict[str, Any]
    ) -> List[str]:
        """スキル習得向上のための推薦を生成"""
        recommendations = []

        if difficulty == 1:  # 基礎
            recommendations.extend(["基本スキルの反復練習", "手順の確認と定着", "簡単な問題での成功体験の積み重ね"])
        elif difficulty == 2:  # 標準
            recommendations.extend(["標準的な問題パターンの習得", "解法手順の体系化", "間違いパターンの分析と対策"])
        else:  # 応用
            recommendations.extend(["基礎スキルの再確認", "段階的な難易度上昇での練習", "応用問題への取り組み前の準備強化"])

        return recommendations

    def _generate_knowledge_recommendations(
        self, subject_name: str, stats: Dict[str, Any]
    ) -> List[str]:
        """知識定着向上のための推薦を生成"""
        return [
    ...
    def _generate_application_recommendations(self, stats: Dict[str, Any]) -> List[str]:
        """応用力向上のための推薦を生成"""
        return ["基礎から応用への段階的学習", "問題解決プロセスの意識化", "多様な問題パターンへの慣れ", "論理的思考力の養成"]
    ...
    def _deduplicate_and_prioritize(
        ...
        sorted_weaknesses = sorted(
            weaknesses, key=lambda x: x["severity_level"], reverse=True
        )
    ...
    def _save_weakness_analysis(
        self, student_id: int, weaknesses: List[Dict[str, Any]]
    ):
        """弱点分析結果をデータベースに保存"""
        try:
            ...
            for weakness_data in weaknesses:
                logger.debug(
                    f"弱点: {weakness_data['category']} - {weakness_data['subcategory']} (重度: {weakness_data['severity_level']})"
                )
    ...
    def get_weakness_summary(self, student_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "last_analysis": active_weaknesses[0].updated_at.isoformat()
