...
class CurriculumUnitService:
    ...
    @staticmethod
    def get_units_for_student(student_id: int, class_id: int = None, 
                            subject_id: int = None, include_progress: bool = True) -> List[Dict]:
        """
        生徒が利用可能な学習単元一覧を取得
        ...
        for unit in units:
            ...
            # 進捗情報の追加
            if include_progress:
                progress = CurriculumUnitService.get_student_progress(
                    student_id, unit.id, class_id
                )
                unit_data['progress'] = progress
                
                # 開始可能かチェック
                can_start, reason = CurriculumUnitService.can_student_start_unit(
                    student_id, unit.id, class_id
                )
    ...
    @staticmethod
    def get_unit_detail(unit_id: int, student_id: int = None, 
                       class_id: int = None) -> Optional[Dict]:
        """
        単元の詳細情報を取得
            ...
            for prereq_id in unit.prerequisites:
                ...
                if prereq_unit:
                    ...
                    if student_id:
                        ...
                        if progress:
                            prereq_data['status'] = progress.get('status', 'not_started')
        ...
        # 紐付けられた問題一覧
        problems = CurriculumUnitService.get_unit_problems(unit_id)
    ...
    def get_unit_problems(unit_id: int) -> List[Dict]:
        ...
        Returns:
            問題情報のリスト
        """
        mappings = db.session.query(UnitItemMapping, BasicKnowledgeItem)\
    ...
    @staticmethod
    def get_student_progress(student_id: int, unit_id: int, 
                           class_id: int = None) -> Optional[Dict]:
        """
        生徒の単元学習進捗を取得
    ...
    @staticmethod
    def can_student_start_unit(student_id: int, unit_id: int, 
                             class_id: int = None) -> Tuple[bool, Optional[str]]:
        """
        生徒が単元を開始できるかチェック
        ...
        if class_id:
            ...
            if settings:
                ...
                if current_units >= settings.max_concurrent_units:
                    return False, f"同時学習可能単元数の上限（{settings.max_concurrent_units}）に達しています"
        
        # 前提条件のチェック
        if unit.prerequisites:
            for prereq_id in unit.prerequisites:
                prereq_progress = CurriculumUnitService.get_student_progress(
                    student_id, prereq_id, class_id
                )
                
                if not prereq_progress or prereq_progress['status'] != 'completed':
                    prereq_unit = CurriculumUnit.query.get(prereq_id)
                    prereq_title = prereq_unit.title if prereq_unit else f"単元{prereq_id}"
                    return False, f"前提単元「{prereq_title}」を完了してください"
                
                # 最低正解率のチェック
                if class_id:
                    settings = ClassLearningSettings.query.filter_by(class_id=class_id).first()
                    if settings and prereq_progress['accuracy_rate'] < float(settings.min_completion_rate):
                        return False, f"前提単元の正解率が不足しています（最低{settings.min_completion_rate}%必要）"
        
        return True, None
    
    @staticmethod
    def select_unit(student_id: int, unit_id: int, class_id: int = None, 
                   notes: str = None) -> Dict:
        """
        生徒が単元を選択
    ...
    @staticmethod
    def update_progress(student_id: int, unit_id: int, class_id: int = None,
                       completed_items: int = None, correct_items: int = None,
                       study_time_minutes: int = None, notes: str = None) -> Dict:
        """
        学習進捗を更新
    ...
    @staticmethod
    def create_class_learning_settings(class_id: int, created_by: int, 
                                     settings: Dict) -> Dict:
        """
        クラス学習設定を作成・更新
    ...
    def get_learning_path(class_id: int, path_id: int = None) -> Optional[Dict]:
        ...
        # パス内の単元を取得
        path_units = db.session.query(LearningPathUnit, CurriculumUnit)\
