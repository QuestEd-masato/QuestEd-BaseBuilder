...
class UnitItemMappingService:
    ...
    def _get_problems_by_unit_attributes(unit_id: int) -> List[Dict]:
        ...
        try:
            ...
            query = BasicKnowledgeItem.query.filter_by(
                subject_id=unit.subject_id, is_active=True
            )
    ...
    @staticmethod
    def calculate_unit_progress(
        student_id: int, unit_id: int
    ) -> Tuple[float, int, int]:
        """
        単元の学習進捗を計算
        ...
        try:
            ...
            progress_percentage = (
                (correct_answers / total_problems * 100) if total_problems > 0 else 0
            )
    ...
    def update_unit_selection_progress(student_id: int, unit_id: int) -> bool:
        ...
        try:
            ...
            (
                ...
                correct_count,
                total_count,
            ) = UnitItemMappingService.calculate_unit_progress(student_id, unit_id)
            ...
            if not selection:
                logger.warning(
                    f"No selection found for student {student_id}, unit {unit_id}"
                )
                return False
            ...
            logger.info(
                f"Updated progress for student {student_id}, unit {unit_id}: {progress_percentage}%"
            )
            return True
        ...
        try:
            ...
            for idx, problem in enumerate(problems):
                try:
                    ...
                    """,
                        {"unit_id": unit_id, "item_id": problem.id, "order_index": idx},
                    )
                    created_count += 1
                except Exception as e:
                    logger.warning(
                        f"Failed to create mapping for problem {problem.id}: {str(e)}"
                    )
    ...
    def batch_create_mappings() -> Dict[str, int]:
        ...
        try:
            ...
            for unit in unmapped_units:
                try:
                    mapping_count = UnitItemMappingService.create_automatic_mappings(
                        unit["id"]
                    )
                    if mapping_count > 0:
