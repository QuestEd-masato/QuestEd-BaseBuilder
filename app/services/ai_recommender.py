...
class AIRecommendationEngine:
    ...
    def _get_recent_recommendations(
        self, student_id: int, recommendation_type: str, hours: int = 24
    ) -> List[Dict[str, Any]]:
        """最近の推薦を取得"""
    ...
    def _collect_learning_data(self, student_id: int) -> Dict[str, Any]:
        ...
        try:
                ...
                data["unit_selections"].append(
                    {
                        ...
                        "difficulty": selection.unit.difficulty
                        if selection.unit
                        else 2,
                        "last_activity": selection.last_activity_at.isoformat()
                    ...
                    data["word_proficiency"].append(
                        {
                            ...
                            "review_date": wp.review_date.isoformat()
                            if wp.review_date
                            else None,
                ...
                data["activity_logs"].append(
                    {
                        ...
                        "reflection_length": len(activity.reflection)
            ...
            active_todos = Todo.query.filter_by(
                student_id=student_id, is_completed=False
            ).count()
    ...
    def _analyze_recent_performance(self, student_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            # 得意科目を特定
            performance["preferred_subjects"] = self._identify_preferred_subjects(
                student_id
            )
    ...
    def _identify_preferred_subjects(self, student_id: int) -> List[str]:
        ...
        try:
            ...
            high_performance_records = (
                ...
                .order_by(desc(ProficiencyRecord.accuracy_rate))
                .limit(3)
                .all()
            )
    ...
    def _parse_ai_response(
        self, ai_response: str, recommendation_type: str, max_recommendations: int
    ) -> List[Dict[str, Any]]:
        """AI応答を解析して推薦リストに変換"""
        try:
            # JSONの抽出を試行
    ...
    def _generate_fallback_recommendations(
        self, student_id: int, recommendation_type: str, max_recommendations: int
    ) -> List[Dict[str, Any]]:
        """フォールバック推薦を生成（ルールベース）"""
        try:
            recommendations = []
            ...
            if recommendation_type == "unit":
                # 未開始の単元から推薦
                unstarted_units = (
                    db.session.query(CurriculumUnit)
                    .filter(
                        ~CurriculumUnit.id.in_(
                            db.session.query(StudentUnitSelection.unit_id).filter_by(
                                student_id=student_id
                            )
                        )
                    )
                ...
                weak_records = (
                    ...
                    .order_by(ProficiencyRecord.accuracy_rate)
                    .limit(max_recommendations)
                    .all()
                )
        ...
        try:
            ai_recommendation = AIRecommendation(
                ...
                confidence_score=sum(
                    rec.get("confidence_score", 0.5) for rec in recommendations
                )
                / len(recommendations)
                if recommendations
                else 0.0,
    ...
    def update_recommendation_settings(
        self, student_id: int, settings_data: Dict[str, Any]
    ):
        """推薦設定を更新"""
        try:
