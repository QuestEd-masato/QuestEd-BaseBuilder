...
from app.models import Curriculum, CurriculumUnit, User, Class, Subject
...
class UnifiedCurriculumService(BaseService):
    ...
    def update_curriculum(self, curriculum_id: int, data: Dict[str, Any]) -> Dict[str, Any]:
        """
        カリキュラム更新
        ...
        try:
            ...
            # バリデーション
            validation_result = self.validator.validate_curriculum_update(curriculum, data)
            if not validation_result['valid']:
                return {
                    'success': False,
                    'message': 'バリデーションエラー',
    ...
    def get_curriculum(self, curriculum_id: int, include_units: bool = False) -> Optional[Dict[str, Any]]:
        """
        カリキュラム取得
        ...
        try:
            ...
            if curriculum.curriculum_data:
                try:
                    curriculum_data['curriculum_data'] = json.loads(curriculum.curriculum_data)
                except:
    ...
    def convert_to_units(self, curriculum_id: int, options: Optional[Dict] = None) -> Dict[str, Any]:
        """
        カリキュラムから単元への変換
...
class CurriculumValidator:
    ...
    def validate_curriculum_update(self, curriculum, data: Dict[str, Any]) -> Dict[str, Any]:
        """カリキュラム更新データのバリデーション"""
        # 基本バリデーション
        ...
        if curriculum.is_active and data.get('is_active') is False:
            ...
            if active_units:
                result['errors'].append("Cannot deactivate curriculum with active units")
                result['valid'] = False
        ...
        try:
            ...
            if options.get('replace_existing', False):
                existing_units = dal.safe_query(
                    CurriculumUnit,
                    filters={'curriculum_id': curriculum.id}
                )
            ...
            for i, unit_data in enumerate(units_data):
                unit = CurriculumUnit(
                    ...
                    subject_id=curriculum.subject_id,
                    school_id=curriculum.class_obj.school_id if curriculum.class_obj else None,
