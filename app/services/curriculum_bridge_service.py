...
class CurriculumBridgeService:
    ...
    @classmethod
    def convert_curriculum_to_units(
        cls, curriculum_id: int, created_by: int
    ) -> Dict[str, any]:
        """
        ...
        try:
            ...
            for index, item in enumerate(items):
                # 既存単元の確認
                existing_unit = next(
                    (u for u in existing_units if u.order_index == index), None
                )
    ...
    @classmethod
    def _create_curriculum_unit(
        cls, curriculum: Curriculum, item: Dict, index: int, created_by: int
    ) -> CurriculumUnit:
        """カリキュラムアイテムから新規単元を作成"""
        ...
        # 難易度推定（カリキュラムのメタデータ優先）
        curriculum_data = (
            json.loads(curriculum.curriculum_data) if curriculum.curriculum_data else {}
        )
        difficulty_level = curriculum_data.get(
            "difficulty_level", cls._estimate_difficulty(item)
        )

        # 学習時間推定（50分コマ基準）
        estimated_minutes = cls._estimate_learning_time_50min(item, curriculum_data)

        # 習熟度判定基準を取得
        mastery_threshold = curriculum_data.get("mastery_threshold", 80)

        # 自由進度モードを取得
        self_paced_mode = curriculum_data.get("self_paced_mode", "flexible")

        # 前提スキルを取得
        prerequisite_skills = curriculum_data.get("prerequisite_skills", "")
        ...
        unit = CurriculumUnit(
            ...
            estimated_classes=round(estimated_minutes / 50, 1),  # 50分コマ基準で計算
            mastery_threshold=mastery_threshold,
            self_paced_mode=self_paced_mode,
            prerequisite_skills=prerequisite_skills,
            order_index=index,
            is_active=True,
            school_id=curriculum.class_obj.school_id
            if curriculum.class_obj and curriculum.class_obj.school_id
            else None,
    ...
    @classmethod
    def _update_curriculum_unit(
        cls, unit: CurriculumUnit, curriculum: Curriculum, item: Dict, index: int
    ):
        """既存単元をアイテム情報で更新"""
        ...
        unit.tags = json.dumps(
            [item.get("phase", ""), item.get("category", ""), "converted"],
            ensure_ascii=False,
        )
    ...
    @classmethod
    def _estimate_learning_time_50min(cls, item: Dict, curriculum_data: Dict) -> int:
        """アイテム情報から学習時間を推定（50分コマ基準）"""
        ...
        total_items = curriculum_data.get(
            "total_items", len(curriculum_data.get("items", []))
        )

        if total_items > 0:
            # 平均的にコマ数を割り当て
            avg_classes_per_item = total_classes / total_items
            return int(avg_classes_per_item * 50)  # 50分/コマ

        # フォールバック: 活動内容の長さから推定（50分コマ基準）
        activity = item.get("activity", "")
        if len(activity) > 200:
            return 100  # 2コマ相当
        elif len(activity) > 100:
    ...
    def sync_curriculum_updates(cls, curriculum_id: int) -> Dict[str, any]:
        ...
        try:
            ...
            if existing_units > 0:
                # 既存単元を更新
                return cls.convert_curriculum_to_units(
                    curriculum_id, curriculum.created_by
                )
            else:
    ...
    def get_conversion_status(cls, curriculum_id: int) -> Dict[str, any]:
        ...
        try:
            ...
            return {
                ...
                "conversion_rate": (converted_units / total_items * 100)
                if total_items > 0
                else 0,
            }
        ...
        try:
            ...
            units = (
                CurriculumUnit.query.filter_by(
                    legacy_curriculum_id=curriculum.id, is_active=True
                )
