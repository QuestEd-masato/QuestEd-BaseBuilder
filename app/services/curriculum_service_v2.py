...
class CurriculumServiceV2:
    ...
    def get_curriculum_items(cls, curriculum_id: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(
                f"Retrieved {len(items)} curriculum items for curriculum {curriculum_id}"
            )
            return items

        except Exception as e:
            logger.error(
                f"Error getting curriculum items for {curriculum_id}: {str(e)}"
            )
            return []

    @classmethod
    def save_curriculum_items(
        cls, curriculum_id: int, items: List[Dict[str, Any]]
    ) -> Tuple[bool, str]:
        """
        カリキュラム項目の保存
        ...
        try:
            ...
            for order_index, item in enumerate(items):
                ...
                """
                    ),
                    {
                        ...
                        if str(item.get("hours", 0)).isdigit()
                        else 0,
            ...
            db.session.execute(
                text(
                    "UPDATE curriculums SET format = 'table', updated_at = NOW() WHERE id = :id"
                ),
                {"id": curriculum_id},
            )
            ...
            db.session.commit()
            logger.info(
                f"Saved {len(items)} curriculum items for curriculum {curriculum_id}"
            )
            ...
            for row in reader:
                ...
                for key, value in row.items():
                    if key and key.strip():
                        normalized_key = header_mapping.get(
                            key.strip(), key.strip().lower()
                        )
    ...
    def get_related_problems(
        cls, category: str, student_id: Optional[int] = None
    ) -> List[Dict[str, Any]]:
        """
        カテゴリに関連する問題を取得
        ...
        try:
            ...
            result = db.session.execute(
                exact_match_query, {"category": category, "student_id": student_id}
            )
            ...
            if not problems:
                ...
                category_words = " ".join(
                    [f"%{word}%" for word in category.split() if len(word) > 1]
                )
            ...
            logger.debug(
                f"Found {len(problems)} related problem categories for '{category}'"
            )
            return problems
    ...
    @classmethod
    def generate_review_problems(
        cls, curriculum_item_id: int, student_id: int
    ) -> List[Dict[str, Any]]:
        """
        復習問題の生成
        ...
        try:
            ...
            result = db.session.execute(
                query, {"category": f"%{item.category}%", "student_id": student_id}
            )
            ...
            logger.debug(
                f"Generated {len(problems)} review problems for item {curriculum_item_id}"
            )
            return problems

        except Exception as e:
            logger.error(
                f"Error generating review problems for item {curriculum_item_id}: {str(e)}"
            )
            return []
    ...
    def get_curriculum_stats(cls, curriculum_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            phases = list(
                set(item.get("phase", "") for item in items if item.get("phase"))
            )
            categories = list(
            ...
            return {
                ...
                "avg_hours_per_item": round(total_hours / len(items), 1)
                if items
                else 0,
            }
            ...
            logger.error(
                f"Error getting curriculum stats for {curriculum_id}: {str(e)}"
            )
            return {"total_items": 0, "total_hours": 0, "phases": [], "categories": []}
