...
class PatternAnalyzerService(BaseService):
    ...
    def analyze_student_patterns(
        self, student_id: int, requester=None
    ) -> Dict[str, Any]:
        """学生の全パターンを分析"""
        ...
        try:
            ...
            results["difficulty_preference"] = self.difficulty_analyzer.analyze(
                student_id
            )
    ...
    def get_student_patterns(self, student_id: int, requester=None) -> Dict[str, Any]:
        ...
        result = {"student_id": student_id, "patterns": {}, "last_updated": None}
        ...
        for pattern in patterns:
            result["patterns"][pattern.pattern_type] = {
                ...
                "last_analyzed": pattern.last_analyzed_at.isoformat()
                if pattern.last_analyzed_at
                else None,
            }
            ...
            if not result[
                "last_updated"
            ] or pattern.last_analyzed_at > datetime.fromisoformat(
                result["last_updated"]
            ):
    ...
    def _save_patterns(self, student_id: int, patterns: Dict[str, Any]):
        ...
        for pattern_type, data in patterns.items():
            ...
            pattern = LearningPattern.query.filter_by(
                student_id=student_id, pattern_type=pattern_type
            ).first()
...
class DifficultyPreferenceAnalyzer:
    ...
    def analyze(self, student_id: int) -> Dict[str, Any]:
        ...
        query = (
            ...
            .join(CurriculumUnit, StudentUnitSelection.unit_id == CurriculumUnit.id)
            .filter(
                and_(
                    StudentUnitSelection.student_id == student_id,
        ...
        for selection, difficulty, estimated_time in query:
            ...
            difficulty_stats[difficulty]["total_time"] += (
                selection.study_time_minutes or 0
            )
            difficulty_stats[difficulty]["progress_sum"] += float(
                selection.progress_percentage or 0
            )
        ...
        for difficulty, stats in difficulty_stats.items():
            ...
            avg_correct_rate = (
                stats["correct_rate"] / stats["attempts"]
                if stats["attempts"] > 0
                else 0
            )
        ...
        return {
            "data": {
                ...
                "recommendation": self._generate_difficulty_recommendation(
                    preferred_difficulty, difficulty_scores
                ),
            },
            "confidence": round(confidence, 2),
    ...
    def _generate_difficulty_recommendation(
        self, preferred_difficulty: Optional[int], difficulty_scores: Dict[str, Any]
    ) -> str:
        """難易度推薦メッセージを生成"""
...
class SubjectStrengthAnalyzer:
    ...
    def analyze(self, student_id: int) -> Dict[str, Any]:
        ...
        subject_activities = (
            ...
            .join(Subject, ChatHistory.subject_id == Subject.id)
            .filter(
                and_(
                    ChatHistory.user_id == student_id,
                    ChatHistory.created_at >= cutoff_date,
            ...
            .group_by(ChatHistory.subject_id, Subject.name)
            .all()
        )
        ...
        subject_scores = defaultdict(
            lambda: {"activity_count": 0, "engagement_score": 0, "consistency_score": 0}
        )
        ...
        # 最強の科目を特定
        strongest_subject = (
            max(final_scores, key=lambda x: final_scores[x]["strength_score"])
            if final_scores
            else None
        )
    ...
    def _generate_subject_recommendations(
        self, subject_scores: Dict[str, Any]
    ) -> List[str]:
        """科目別推薦を生成"""
        ...
        sorted_subjects = sorted(
            subject_scores.items(), key=lambda x: x[1]["strength_score"], reverse=True
        )
...
class LearningStyleAnalyzer:
        ...
        return {
            "data": {
                ...
                "recommendations": self._generate_style_recommendations(
                    learning_style, avg_session_time
                ),
            },
            "confidence": round(confidence, 2),
    ...
    def _analyze_chat_sessions(
        self, student_id: int, cutoff_date: datetime
    ) -> List[int]:
        """チャットセッション時間を分析"""
        chats = (
        ...
        for chat in chats[1:]:
            ...
            if time_diff > 30:  # 30分以上空いたら新しいセッション
                session_duration = (
                    last_timestamp - current_session_start
                ).total_seconds() / 60
    ...
    def _classify_learning_style(
        self, avg_time: float, session_times: List[int]
    ) -> str:
        """学習スタイルを分類"""
        if avg_time < 15:
            return "短時間集中型"
    ...
    def _analyze_learning_frequency(
        self, student_id: int, cutoff_date: datetime
    ) -> Dict[str, Any]:
        """学習頻度を分析"""
        # 日別の学習日数を計算
