...
class SuperMemoAlgorithm:
    ...
    def calculate_next_review(
        self, card: SpacedRepetitionCard, result: ReviewResult
...
class AdaptiveDifficultyAdjuster:
    ...
    def adjust_difficulty(
        self, current_accuracy: float, current_difficulty: float, sample_size: int = 1
    ) -> float:
        """
        正答率に基づいて難易度を調整
...
class SpacedRepetitionEngine:
    ...
    def create_review_set(
        ...
        try:
            ...
            review_set = ReviewSet(
                ...
                description=self._generate_review_description(
                    review_type, focus_weaknesses
                ),
    ...
    def _select_review_items(
        self, student_id: int, target_count: int, focus_weaknesses: bool
    ) -> List[Dict[str, Any]]:
        """復習対象問題を選択"""
        review_items = []
        ...
        try:
            ...
            if focus_weaknesses:
                weakness_items = self._get_weakness_focused_items(
                    student_id, max(target_count - len(review_items), 0)
                )
            ...
            if len(review_items) < target_count:
                random_items = self._get_random_review_items(
                    student_id, target_count - len(review_items)
                )
    ...
    def _get_spaced_repetition_items(self, student_id: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for record in proficiency_records:
                # カテゴリ内の問題を取得
                problems = (
                    BasicKnowledgeItem.query.filter_by(category_id=record.category_id)
                    .limit(3)
                    .all()
                )  # カテゴリあたり最大3問
    ...
    def _get_weakness_focused_items(
        self, student_id: int, max_count: int
    ) -> List[Dict[str, Any]]:
        """弱点に焦点を当てた問題を取得"""
        items = []
    ...
    def _get_random_review_items(
        ...
        try:
            # 過去に学習した問題からランダムに選択
            studied_categories = (
                db.session.query(ProficiencyRecord.category_id)
                .filter_by(student_id=student_id)
                .subquery()
            )

            problems = (
                db.session.query(BasicKnowledgeItem)
                .filter(BasicKnowledgeItem.category_id.in_(studied_categories))
    ...
    def _get_or_create_card(
        self, student_id: int, problem_id: int, proficiency_record: ProficiencyRecord
    ) -> SpacedRepetitionCard:
        """間隔反復カードを取得または作成"""
    ...
    def _estimate_initial_difficulty(
        self, proficiency_record: ProficiencyRecord
    ) -> float:
        """初期難易度を推定"""
    ...
    def _generate_review_description(
        self, review_type: str, focus_weaknesses: bool
    ) -> str:
        """復習セットの説明を生成"""
        descriptions = {
    ...
    def _calculate_quality_score(
        self, is_correct: bool, confidence_level: int, time_spent_seconds: int
    ) -> int:
        """品質スコア（1-5）を計算"""
        if not is_correct:
            return 1  # 不正解は最低スコア
        ...
        try:
            ...
            for review_set in recent_sets:
                items = ReviewSetItem.query.filter_by(
                    review_set_id=review_set.id, is_completed=True
                ).all()
    ...
    def _calculate_review_streak(self, student_id: int) -> int:
        ...
        try:
            # 最近の復習セット完了日を取得
            completed_sets = (
                ReviewSet.query.filter(
                    and_(
                        ReviewSet.student_id == student_id,
                        ReviewSet.status == "completed",
                    )
                )
                .order_by(desc(ReviewSet.created_at))
                .limit(30)
                .all()
            )
    ...
    def _get_recent_performance(self, student_id: int, days: int) -> Dict[str, Any]:
        ...
        try:
            ...
            recent_items = (
                db.session.query(ReviewSetItem)
                .join(ReviewSet)
                .filter(
                    and_(
                        ReviewSet.student_id == student_id,
            ...
            return {
                ...
                "avg_time_per_problem": total_time / len(recent_items)
                if recent_items
                else 0,
            }
