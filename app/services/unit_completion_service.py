...
class UnitCompletionService:
    ...
    def request_completion(
        ...
        try:
            ...
            class_settings = None
            if class_id:
                class_settings = ClassLearningSettings.query.filter_by(
                    class_id=class_id
                ).first()
            ...
            if class_settings and not class_settings.require_teacher_approval:
                ...
                class_settings
                and class_settings.auto_approve_threshold
                and selection.progress_percentage
                >= class_settings.auto_approve_threshold
            ):
                auto_approve = True
            ...
            logger.info(
                f"Unit completion requested: student_id={student_id}, unit_id={unit_id}, auto_approve={auto_approve}"
            )
    ...
    def approve_completion(
        selection_id: int, teacher_id: int, comments: str = None
    ) -> Dict[str, any]:
        """
        単元完了申請を承認
        ...
        try:
            ...
            logger.info(
                f"Unit completion approved: selection_id={selection_id}, teacher_id={teacher_id}"
            )
    ...
    def reject_completion(
        selection_id: int, teacher_id: int, reason: str
    ) -> Dict[str, any]:
        """
        単元完了申請を却下
        ...
        try:
            ...
            logger.info(
                f"Unit completion rejected: selection_id={selection_id}, teacher_id={teacher_id}"
            )
    ...
    def get_pending_approvals(
        teacher_id: int, class_id: int = None, limit: int = 50
    ) -> Dict[str, any]:
        """
        承認待ち申請一覧を取得
        ...
        try:
            ...
            query = (
                ...
                .join(User, StudentUnitSelection.student_id == User.id)
                .filter(StudentUnitSelection.approval_status == "pending")
            )
            ...
            # 申請日時の降順でソート
            selections = (
                query.order_by(desc(StudentUnitSelection.completion_request_date))
                .limit(limit)
                .all()
            )
                ...
                approval_list.append(
                    {
                        ...
                        "days_pending": (
                            datetime.utcnow() - selection.completion_request_date
                        ).days,
                    }
                )
        ...
        try:
            ...
            total_requests = StudentUnitSelection.query.filter(
                and_(
                    ...
                    StudentUnitSelection.approval_status.in_(
    ...
    @staticmethod
    def batch_approve(
        selection_ids: List[int], teacher_id: int, comments: str = None
    ) -> Dict[str, any]:
        """
        一括承認処理
        ...
        try:
            ...
            for selection_id in selection_ids:
                result = UnitCompletionService.approve_completion(
                    selection_id, teacher_id, comments
                )
                if result["success"]:
                    approved_count += 1
                else:
    ...
    @staticmethod
    def get_student_completion_history(
        ...
        try:
            ...
            approved_selections = (
                ...
                .order_by(desc(StudentUnitSelection.approved_at))
                .limit(limit)
                .all()
            )
            ...
            for selection in approved_selections:
                unit_data = selection.curriculum_unit.to_dict()
                approver_name = (
                    selection.approver.full_name if selection.approver else "自動承認"
                )
