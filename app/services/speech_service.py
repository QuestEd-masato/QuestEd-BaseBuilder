...
from app.models import SpeechTranscription, User
...
class SpeechService:
    ...
    def record_transcription(
        ...
        # 統計情報更新
        SpeechService._update_statistics(
            student_id,
            {
                "is_successful": True,
    ...
    def get_transcription_history(
        ...
        # ページング
        transcriptions = (
            query.order_by(SpeechTranscription.created_at.desc())
            .offset(offset)
            .limit(limit)
            .all()
        )
    ...
    def get_statistics(student_id: int, days: int = 30) -> Dict:
        ...
        successful_inputs = len(
            [
                t
                for t in transcriptions
                if t.transcription and len(t.transcription.strip()) > 0
            ]
        )
        ...
        most_used_context = (
            max(context_usage.items(), key=lambda x: x[1])[0] if context_usage else None
        )
        ...
        return {
            ...
            "summary": {
                ...
                "success_rate": (successful_inputs / total_inputs * 100)
                if total_inputs > 0
                else 0.0,
    ...
    def record_error(
        ...
        # 統計情報更新（失敗として記録）
        SpeechService._update_statistics(
            student_id, {"is_successful": False, "context": input_context}
        )
    ...
    def get_session_transcriptions(session_id: str) -> List[Dict]:
        ...
        """
        transcriptions = (
            SpeechTranscription.query.filter_by(session_id=session_id)
            .order_by(SpeechTranscription.created_at)
            .all()
        )
    ...
    def get_usage_analytics(student_id: int) -> Dict:
        ...
        context_query = (
            db.session.query(
                ...
                db.func.avg(SpeechTranscription.confidence_score).label(
                    "avg_confidence"
                ),
            )
            .filter(
            ...
            .group_by(SpeechTranscription.input_context)
            .all()
        )
        ...
        time_query = (
            ...
            .group_by(db.func.extract("hour", SpeechTranscription.created_at))
            .all()
        )

        time_stats = [
            {"hour": int(row.hour), "usage_count": row.count} for row in time_query
        ]
    ...
    def _generate_improvement_suggestions(
        summary: Dict, context_stats: List[Dict]
    ) -> List[str]:
        """
        音声入力改善提案を生成
