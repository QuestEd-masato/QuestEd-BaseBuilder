...
class WeaknessPatternAnalyzer:
    ...
    def _analyze_error_patterns(self, answer_records: List[Dict]) -> Dict:
        ...
        return {
            ...
            'max_consecutive_errors': max(consecutive_errors) if consecutive_errors else 0,
            'avg_error_streak': statistics.mean(consecutive_errors) if consecutive_errors else 0
        }
    ...
    def _analyze_time_patterns(
        ...
        performance_variance = statistics.variance(hourly_variance) if len(hourly_variance) > 1 else 0
    ...
    def _analyze_difficulty_patterns(
        ...
        deviation = sum(abs(a - e) for a, e in zip(difficulty_gradient, expected_gradient))
        ...
        return {
            ...
            'recommended_difficulty': self._recommend_difficulty_level(difficulty_statistics)
        }
    ...
    def _analyze_learning_curves(self, answer_records: List[Dict]) -> Dict:
        ...
        # 改善率を計算
        improvement_rate = (window_accuracies[-1] - window_accuracies[0]) / len(window_accuracies)
    ...
    def _analyze_concept_patterns(self, answer_records: List[Dict]) -> Dict:
        ...
        return {
            ...
            'weak_concepts': sorted(weak_concepts, key=lambda x: x['error_rate'], reverse=True),
            'concept_mastery_distribution': self._calculate_concept_mastery_distribution(concept_errors)
        }
    ...
    def _analyze_engagement_patterns(
        ...
        # エンゲージメントの一貫性を評価
        engagement_scores = [
            data['activities'] + data['answers'] 
            for data in daily_engagement.values()
        ]
        
        consistency = 1 - (statistics.stdev(engagement_scores) / statistics.mean(engagement_scores)) \
            if len(engagement_scores) > 1 and statistics.mean(engagement_scores) > 0 else 0
        ...
        return {
            ...
            'avg_daily_engagement': statistics.mean(engagement_scores) if engagement_scores else 0,
    ...
    def _identify_weakness_clusters(
        ...
        for level, diff_stats in statistics.get('difficulty_statistics', {}).items():
            if diff_stats['mastery_status'] == 'needs_practice' and diff_stats['total_attempts'] >= 5:
                clusters.append({
                    'type': 'difficulty',
    ...
    def _detect_fatigue_pattern(self, answer_records: List[Dict]) -> Dict:
        ...
        shows_fatigue = sum(fatigue_indicators) / len(fatigue_indicators) > 0.5 if fatigue_indicators else False
    ...
    def _detect_difficulty_jumps(self, answer_records: List[Dict]) -> List[Dict]:
        ...
        for i in range(1, len(answer_records)):
            prev_difficulty = answer_records[i-1].get('problem', {}).get('difficulty_level', 1)
            curr_difficulty = answer_records[i].get('problem', {}).get('difficulty_level', 1)
    ...
    def _create_time_windows(self, records: List[Dict], window_size: int) -> List[List[Dict]]:
        """時間窓を作成"""
        if not records:
            return []
    ...
    def _classify_engagement_pattern(self, daily_engagement: Dict) -> str:
        ...
        active_days = sum(1 for data in daily_engagement.values() 
    ...
    def _find_peak_days(self, daily_engagement: Dict) -> List[int]:
        ...
        return [day for day, total in weekday_totals.items() 
                if total >= max_activity * 0.8]
    ...
    def _split_into_sessions(self, answer_records: List[Dict]) -> List[List[Dict]]:
        ...
        for i in range(1, len(answer_records)):
            time_diff = (answer_records[i]['created_at'] - 
                        answer_records[i-1]['created_at']).total_seconds()
