...
class BaseBuilderWeaknessAnalyzer:
    ...
    def _analyze_word_proficiency(
        self, word_proficiencies: List[Dict]
    ) -> List[Weakness]:
        """単語習熟度から弱点を分析"""
        weaknesses = []
    ...
    def _analyze_category_performance(
        self, category_performance: List[Dict]
    ) -> List[Weakness]:
        """カテゴリ別パフォーマンスから弱点を分析"""
        weaknesses = []
    ...
    def _analyze_vocabulary_patterns(
        self, word_proficiencies: List[Dict]
    ) -> List[Weakness]:
        """語彙学習パターンから弱点を分析"""
        weaknesses = []
    ...
    def _analyze_text_proficiency(
        self, text_proficiencies: List[Dict]
    ) -> List[Weakness]:
        """テキスト習熟度から弱点を分析"""
        weaknesses = []
        ...
        # 未完了テキストの分析
        incomplete_texts = [
            t for t in text_proficiencies if t.get("completion_rate", 0) < 1.0
        ]

        if len(incomplete_texts) > 5:
            avg_completion = statistics.mean(
                [t.get("completion_rate", 0) for t in incomplete_texts]
            )
