...
class WeaknessSeverityEvaluator:
    ...
    def evaluate_weaknesses(
        ...
        try:
            ...
            # 概念パターンから弱点を抽出
            weaknesses.extend(
                self._evaluate_concept_weaknesses(patterns.get("concept_patterns", {}))
            )
    ...
    def _evaluate_error_pattern_weaknesses(
    ...
    def _evaluate_concept_weaknesses(self, concept_patterns: Dict) -> List[Weakness]:
        ...
        for weak_concept in concept_patterns.get("weak_concepts", []):
            ...
            weakness = Weakness(
                ...
                affected_areas=self._get_concept_affected_areas(
    ...
    def _evaluate_category_weaknesses(
        self, category_statistics: Dict
    ) -> List[Weakness]:
        """カテゴリ別の弱点を評価"""
        weaknesses = []
    ...
    def _evaluate_difficulty_weaknesses(
        self, difficulty_patterns: Dict, difficulty_statistics: Dict
    ) -> List[Weakness]:
        """難易度関連の弱点を評価"""
        weaknesses = []
        ...
        for level, stats in difficulty_statistics.items():
            if (
                stats["mastery_status"] == "needs_practice"
                and stats["total_attempts"] >= 10
            ):
                severity = 1.0 - stats["accuracy_rate"]
    ...
    def _evaluate_learning_curve_weaknesses(
        self, learning_curve_patterns: Dict
    ) -> List[Weakness]:
        """学習曲線から弱点を評価"""
        weaknesses = []
        ...
        if curve_type == "declining":
            weakness = Weakness(
                ...
                confidence=0.7,
                evidence_count=len(
                    learning_curve_patterns.get("weekly_accuracies", [])
                ),
        ...
        elif curve_type == "plateau" and learning_curve_patterns.get(
    ...
    def _deduplicate_and_prioritize(self, weaknesses: List[Weakness]) -> List[Weakness]:
        ...
        for weakness in weaknesses:
            ...
            if (
                key not in unique_weaknesses
                or weakness.severity > unique_weaknesses[key].severity
            ):
