...
class WeaknessRecommendationGenerator:
    ...
    def generate_recommendations(
        self, weaknesses: List[Weakness], learning_data: Dict[str, Any]
    ) -> List[Recommendation]:
        """
        弱点に基づいて推奨事項を生成
        ...
        recommendations.append(
            Recommendation(
                ...
                resources=[
                    {
                        "type": "textbook",
                        "title": f"{concept}基礎テキスト",
                        "url": f"#text-{concept}",
                    },
                    {
                        "type": "video",
                        "title": f"{concept}解説動画",
                        "url": f"#video-{concept}",
                    },
                ],
                prerequisites=[],
        ...
        recommendations.append(
            Recommendation(
                ...
                resources=[
                    {
                        "type": "problems",
                        "title": f"{concept}練習問題集",
                        "url": f"#problems-{concept}",
                    }
                ],
    ...
    def _generate_category_recommendations(
        self, weakness: Weakness, learning_data: Dict[str, Any]
    ) -> List[Recommendation]:
        """カテゴリ別弱点に対する推奨事項"""
        recommendations = []
        ...
        recommendations.append(
            Recommendation(
                ...
                resources=[
                    {
                        "type": "learning_path",
                        "title": f"カテゴリ{category_id}マスタープラン",
                        "url": f"#path-{category_id}",
                    }
                ],
                prerequisites=[],
                ...
                resources=[
                    {
                        "type": "infographic",
                        "title": "概念マップ",
                        "url": f"#map-{category_id}",
                    },
                    {
                        "type": "flashcards",
                        "title": "暗記カード",
                        "url": f"#cards-{category_id}",
                    },
                ],
                prerequisites=[],
    ...
    def _deduplicate_and_prioritize_recommendations(
        self, recommendations: List[Recommendation], weaknesses: List[Weakness]
    ) -> List[Recommendation]:
        """推奨事項の重複除去と優先順位付け"""
        # 重複除去
