...
class WeaknessPersistenceService:
    ...
    def get_recent_analysis(
        self, student_id: int, max_age_hours: Optional[int] = None
    ...
    def save_analysis(self, analysis_result: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            new_analysis = WeaknessAnalysis(
                ...
                weaknesses_json=self._serialize_weaknesses(
                ...
                statistics_json=json.dumps(
                    analysis_result.get("statistics", {}), default=str
                ),
                patterns_json=json.dumps(
                    analysis_result.get("patterns", {}), default=str
                ),
                is_active=True,
    ...
    def get_analysis_history(
        ...
        try:
            analyses = (
                WeaknessAnalysis.query.filter_by(student_id=student_id)
                .order_by(desc(WeaknessAnalysis.created_at))
                .limit(limit)
                .all()
            )
    ...
    def get_weakness_trends(
        self, student_id: int, weakness_category: str, days_back: int = 30
    ) -> List[Dict[str, Any]]:
        """
        特定の弱点のトレンドを取得
        ...
        try:
            ...
            analyses = (
                ...
                .order_by(WeaknessAnalysis.created_at)
                .all()
            )
            ...
            for analysis in analyses:
                ...
                # 指定カテゴリの弱点を探す
                category_weaknesses = [
                    w for w in weaknesses if w.get("category") == weakness_category
                ]

                if category_weaknesses:
                    avg_severity = sum(
                        w["severity"] for w in category_weaknesses
                    ) / len(category_weaknesses)
                    trends.append(
                        {
    ...
    def _deserialize_analysis(
        self, analysis_record: "WeaknessAnalysis"
    ) -> Dict[str, Any]:
        """データベースレコードから分析結果を復元"""
        try:
            ...
            statistics = (
                json.loads(analysis_record.statistics_json)
                if analysis_record.statistics_json
                else {}
            )
            patterns = (
                json.loads(analysis_record.patterns_json)
                if analysis_record.patterns_json
                else {}
            )
            ...
            for weakness in weaknesses:
                if "last_occurrence" in weakness:
                    weakness["last_occurrence"] = datetime.fromisoformat(
                        weakness["last_occurrence"]
                    )
