...
class WeaknessStatisticsCalculator:
    ...
    def _calculate_answer_statistics(self, answer_records: List[Dict]) -> Dict:
        ...
        response_times = [r['response_time'] for r in answer_records if r.get('response_time')]
        
        return {
            'total_answers': len(answer_records),
            'correct_answers': correct_count,
            'accuracy_rate': correct_count / len(answer_records) if answer_records else 0,
    ...
    def _calculate_unit_statistics(self, unit_progress: List[Dict]) -> Dict:
        ...
        in_progress_units = sum(1 for u in unit_progress if u['status'] == 'in_progress')
        
        progress_values = [u['progress_percentage'] for u in unit_progress if u['progress_percentage']]
        ...
        return {
            ...
            'median_progress': statistics.median(progress_values) if progress_values else 0
        }
    ...
    def _calculate_activity_statistics(self, activity_logs: List[Dict]) -> Dict:
        ...
        return {
            ...
            'avg_activities_per_day': statistics.mean(daily_activities.values()) if daily_activities else 0,
            'activity_streak': streak,
            'max_daily_activities': max(daily_activities.values()) if daily_activities else 0
        }
    ...
    def _calculate_basebuilder_statistics(self, basebuilder_data: Dict) -> Dict:
        ...
        if not word_proficiencies:
            ...
            weak_words_count = sum(1 for wp in word_proficiencies if wp['mastery_level'] < 3)
