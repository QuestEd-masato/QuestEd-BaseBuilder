...
class DailyReportService:
    ...
    def generate_all_reports(self):
        ...
        with self.app.app_context():
            try:
                ...
                logging.info(
                    f"Daily reports generated: {student_count} students, {teacher_count} teachers"
                )
                return (
                    True,
                    f"Generated reports for {student_count} students and {teacher_count} teachers",
                )
    ...
    def generate_student_reports(self):
        ...
        for student in students:
            try:
                ...
                if (
                    report_data["total_questions"] > 0
                    or report_data["total_activities"] > 0
                ):
            ...
            try:
                for class_obj in teacher.classes_teaching:
                    ...
                    if report_data["active_students"] > 0:
                        success = self._send_teacher_report(
                            teacher, class_obj, report_data
                        )
                        if success:
                            sent_count += 1
                            logging.info(
                                f"Teacher report sent to {teacher.username} for class {class_obj.name}"
                            )
            except Exception as e:
    ...
    def _collect_student_data(self, student):
        ...
        chats = (
            ...
            .order_by(ChatHistory.subject_id)
            .all()
        )

        # 活動記録
        activities = ActivityLog.query.filter(
            ActivityLog.student_id == student.id, ActivityLog.date == self.today
        ).all()

        # 目標進捗
        goals = Goal.query.filter_by(student_id=student.id, is_completed=False).all()
        ...
        for chat in chats:
            ...
            subject_data[subject_id]["questions"].append(
                {"question": chat.message, "timestamp": chat.created_at}
            )
        ...
        for subject_id, data in subject_data.items():
            if data["questions"]:
                data["summary"] = self._generate_ai_summary(
                    data["subject"].name if data["subject"] else "一般", data["questions"]
                )
                # HTMLエスケープして安全性を確保
                if "summary" in data:
    ...
    def _generate_ai_summary(self, subject_name, questions):
        ...
        question_texts = "\n".join(
            [f"- {q['question']}" for q in questions[:5]]
        )  # 最大5問
