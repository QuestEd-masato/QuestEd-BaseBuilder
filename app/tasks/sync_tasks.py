...
if CELERY_AVAILABLE:
    ...
    def execute_curriculum_sync(self, curriculum_id: int, trigger_type: str = 'manual', user_id: Optional[int] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            # タスク開始ログ
            logger.info(f"Starting background curriculum sync: {curriculum_id} (trigger: {trigger_type})")
            ...
            if sync_result.get('success'):
                ...
                logger.info(f"Background curriculum sync completed successfully: {curriculum_id}")
                ...
                logger.error(f"Background curriculum sync failed: {curriculum_id} - {error_message}")
            ...
            error_msg = str(e)
            logger.error(f"Background curriculum sync error: {curriculum_id} - {error_msg}", exc_info=True)
    ...
    def batch_curriculum_sync(self, curriculum_ids: list, trigger_type: str = 'manual', user_id: Optional[int] = None) -> Dict[str, Any]:
        """
        ...
        """
        try:
            logger.info(f"Starting batch curriculum sync: {len(curriculum_ids)} curricula (trigger: {trigger_type})")
            ...
            for i, curriculum_id in enumerate(curriculum_ids):
                try:
                    ...
                    error_msg = str(e)
                    logger.error(f"Batch sync error for curriculum {curriculum_id}: {error_msg}")
            ...
            logger.info(f"Batch curriculum sync completed: {successful}/{total_curricula} successful")
    ...
    def scheduled_sync_check() -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Scheduled sync check completed: {check_result.get('message', 'No message')}")
    ...
    def execute_curriculum_sync(curriculum_id: int, trigger_type: str = 'manual', user_id: Optional[int] = None) -> Dict[str, Any]:
        """同期フォールバック処理（同期実行）"""
    ...
    def batch_curriculum_sync(curriculum_ids: list, trigger_type: str = 'manual', user_id: Optional[int] = None) -> Dict[str, Any]:
        """一括同期フォールバック処理（同期実行）"""
    ...
    def scheduled_sync_check() -> Dict[str, Any]:
        ...
        logger.warning("Celery not available - executing scheduled sync check synchronously")
...
class SyncTaskManager:
    ...
    @staticmethod
    def start_background_sync(curriculum_id: int, trigger_type: str = 'manual', user_id: Optional[int] = None) -> Dict[str, Any]:
        """
        バックグラウンド同期の開始
    ...
    @staticmethod
    def start_batch_sync(curriculum_ids: list, trigger_type: str = 'manual', user_id: Optional[int] = None) -> Dict[str, Any]:
        """
        一括バックグラウンド同期の開始
