...
def create_class():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            return redirect(
                url_for("teacher_class_management.class_details", class_id=new_class.id)
            )
...
def class_details(class_id):
    ...
    # マイルストーンを取得
    milestones = (
        Milestone.query.filter_by(class_id=class_id)
...
@class_management_bp.route(
    "/class/<int:class_id>/add_students", methods=["GET", "POST"]
)
@login_required
@teacher_required
def add_students(class_id):
    ...
    enrolled_student_ids = [
        e.student_id for e in ClassEnrollment.query.filter_by(class_id=class_id).all()
    ]
    ...
    return render_template(
        "add_students.html", class_obj=class_obj, available_students=available_students
    )
...
@class_management_bp.route(
    "/class/<int:class_id>/import_students", methods=["GET", "POST"]
)
@login_required
@teacher_required
...
@class_management_bp.route(
    "/class/<int:class_id>/milestone/create", methods=["GET", "POST"]
)
@login_required
@teacher_required
def create_milestone(class_id):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            return redirect(
                url_for("teacher_class_management.view_class", class_id=class_id)
                + "#milestones"
            )

        except Exception as e:
            db.session.rollback()
            flash(f"マイルストーンの作成中にエラーが発生しました: {str(e)}")
            logging.error(f"Milestone creation error: {str(e)}")

    return render_template("create_milestone.html", class_obj=class_obj)


@class_management_bp.route(
    "/milestone/<int:milestone_id>/edit", methods=["GET", "POST"]
)
@login_required
@teacher_required
def edit_milestone(milestone_id):
    ...
    if request.method == "POST":
        ...
        return redirect(
            url_for("teacher_class_management.view_class", class_id=class_obj.id)
            + "#milestones"
        )
...
@class_management_bp.route("/milestone/<int:milestone_id>/delete")
@login_required
@teacher_required
...
def view_milestone(milestone_id):
    """マイルストーン詳細表示"""
    from app.models import Milestone, StudentMilestone

    milestone = Milestone.query.get_or_404(milestone_id)
    class_obj = milestone.class_obj

    # 権限チェック（教師または登録されている生徒のみ）
    if current_user.role == "teacher":
        if class_obj.teacher_id != current_user.id:
            flash("このマイルストーンを表示する権限がありません。")
    ...
    if current_user.role == "teacher":
        submissions = (
            db.session.query(StudentMilestone, User)
            .join(User, StudentMilestone.student_id == User.id)
            .filter(StudentMilestone.milestone_id == milestone_id)
            .all()
        )
