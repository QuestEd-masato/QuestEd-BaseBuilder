...
from app.models import (
    db, User, Class, ClassEnrollment, CurriculumUnit, 
    StudentUnitSelection, ClassLearningSettings
)
...
def pending_unit_approvals():
    ...
    try:
        ...
        stats = {
            ...
            'avg_per_class': round(total_pending / classes_with_pending, 1) if classes_with_pending > 0 else 0
        }
...
@approval_workflow_bp.route('/api/approval/<int:selection_id>/approve', methods=['POST'])
@login_required
@teacher_required
...
def pending_count():
    ...
    try:
        ...
        return jsonify({
            'success': False,
            'error': f'件数取得に失敗しました: {str(e)}'
        }), 500

@approval_workflow_bp.route('/class/<int:class_id>/approval-settings', methods=['GET', 'POST'])
@login_required
@teacher_required
def approval_settings(class_id):
    ...
    if request.method == 'POST':
        try:
            # 設定更新
            settings.require_teacher_approval = request.form.get('require_teacher_approval') == 'on'
            settings.auto_approve_threshold = float(request.form.get('auto_approve_threshold', 90.0))
            settings.approval_comment_required = request.form.get('approval_comment_required') == 'on'
            settings.allow_resubmission = request.form.get('allow_resubmission') == 'on'
            
            # バリデーション
            if settings.auto_approve_threshold < 0 or settings.auto_approve_threshold > 100:
                flash('自動承認閾値は0-100の範囲で入力してください。', 'error')
                return render_template('approval_settings.html', 
                                     class_obj=class_obj, 
                                     settings=settings)
...
def get_approval_settings(class_id):
    ...
    try:
        ...
        return jsonify({
            'success': False,
            'error': f'設定取得に失敗しました: {str(e)}'
        }), 500

@approval_workflow_bp.route('/api/class/<int:class_id>/approval-settings/update', methods=['POST'])
@login_required
@teacher_required
    ...
    try:
        ...
        if 'auto_approve_threshold' in data:
            ...
            if 0 <= threshold <= 100:
                ...
                return jsonify({
                    'success': False,
                    'error': '自動承認閾値は0-100の範囲で入力してください'
                }), 400
