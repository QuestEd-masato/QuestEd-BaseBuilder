...
from app.models import BasicKnowledgeItem, Class, Curriculum, CurriculumUnit, db
...
@synchronization_bp.route("/sync-all", methods=["POST"])
@login_required
@teacher_required
def sync_all_curriculums():
    ...
    try:
        ...
        curriculums = Curriculum.query.filter_by(
            teacher_id=current_user.id, is_converted_to_units=True
        ).all()
        ...
        flash(
            f"{success_count}/{len(curriculums)}個のカリキュラムの同期が完了しました。",
...
def get_sync_status(curriculum_id):
    ...
    try:
        ...
        return jsonify({"success": False, "error": f"ステータス取得に失敗しました: {str(e)}"}), 500
...
def get_curriculum_sync_stats(curriculum_id):
    ...
    try:
        ...
        latest_sync = (
            SyncLog.query.filter_by(curriculum_id=curriculum_id)
        ...
        stats = {
            ...
            "last_sync_date": latest_sync.created_at.isoformat()
            if latest_sync
            else None,
        ...
        if auto_sync_settings:
            ...
            stats[
                "sync_on_curriculum_update"
            ] = auto_sync_settings.sync_on_curriculum_update
            stats["last_sync_at"] = (
                auto_sync_settings.last_sync_at.isoformat()
                if auto_sync_settings.last_sync_at
                else None
            )
        ...
        return jsonify({"success": False, "error": f"統計取得に失敗しました: {str(e)}"}), 500


@synchronization_bp.route("/integrated-management")
@login_required
@teacher_required
def integrated_management():
    ...
    try:
        ...
        for class_obj in classes:
            curriculums = Curriculum.query.filter_by(
                class_id=class_obj.id, teacher_id=current_user.id
            ).all()
            ...
            for curriculum in curriculums:
                conversion_status = CurriculumBridgeService.get_conversion_status(
                    curriculum.id
                )
                ...
                if conversion_status.get("is_converted", False):
                    ...
                    class_data["stats"]["total_units"] += conversion_status.get(
                        "converted_units", 0
                    )
            ...
            # 全体統計に加算
            integrated_data["summary"]["total_curriculums"] += class_data["stats"][
                "total_curriculums"
            ]
            integrated_data["summary"]["converted_curriculums"] += class_data["stats"][
                "converted_count"
            ]
            integrated_data["summary"]["total_units"] += class_data["stats"][
                "total_units"
            ]
...
@synchronization_bp.route("/api/realtime-stats")
@login_required
@teacher_required
def get_realtime_stats():
    ...
    try:
        ...
        today_syncs = (
            ...
            .join(Curriculum)
            .filter(Curriculum.teacher_id == current_user.id)
            .count()
        )
...
@synchronization_bp.route("/api/sync-overview-stats")
@login_required
@teacher_required
    ...
    try:
        ...
        stats = {
            ...
            "conversion_rate": round(
                (converted_curriculums / total_curriculums) * 100, 1
            )
            if total_curriculums > 0
            else 0,
        ...
        return jsonify({"success": False, "error": f"概要統計取得に失敗しました: {str(e)}"}), 500


@synchronization_bp.route(
    "/curriculum/<int:curriculum_id>/auto-sync-settings", methods=["GET", "POST"]
)
@login_required
@teacher_required
def auto_sync_settings(curriculum_id):
    ...
    if request.method == "POST":
        try:
            ...
            settings.sync_on_curriculum_update = (
                request.form.get("sync_on_curriculum_update") == "on"
            )
            settings.sync_on_item_change = (
                request.form.get("sync_on_item_change") == "on"
            )

            # 競合解決戦略
            conflict_resolution = request.form.get(
    ...
    return render_template(
        "auto_sync_settings.html", curriculum=curriculum, settings=settings
    )
...
def get_curriculum_problems(curriculum_id):
    ...
    try:
        ...
        # 教科と難易度に基づいて関連問題を検索
        problems = (
            BasicKnowledgeItem.query.filter_by(subject_id=curriculum.subject_id)
            .limit(50)
            .all()
        )
        ...
        return jsonify({"success": False, "error": f"問題取得に失敗しました: {str(e)}"}), 500
