    ...
    if request.method == "POST":
        ...
        for student_id in selected_student_ids:
            ...
            try:
                ...
                todos = Todo.query.filter_by(student_id=student.id).all()
                ...
                evaluations.append({"student": student, "evaluation": evaluation_text})

            except Exception as e:
                logging.error(
                    f"Evaluation generation error for student {student.id}: {str(e)}"
                )
                evaluations.append(
                    {"student": student, "evaluation": f"評価生成中にエラーが発生しました: {str(e)}"}
                )
        ...
        return render_template(
            "evaluation_results.html", evaluations=evaluations, class_obj=class_obj
        )
...
def generate_student_report(class_id, student_id):
    ...
    try:
        ...
        evaluations = (
            StudentEvaluation.query.filter_by(
        ...
        activity_summary = generate_activity_summary(
            activities[:10], student.full_name or student.username  # 最新10件の活動
        )
...
def teacher_themes():
    ...
    return render_template(
        "teacher_themes.html", classes_with_themes=classes_with_themes
    )
        ...
        evaluation_data.append(
            {
                ...
                "class_name": evaluation.class_obj.name
                if evaluation.class_obj
                else "不明",
            }
        )
...
@student_evaluation_bp.route(
    "/evaluation/<int:evaluation_id>/edit", methods=["GET", "POST"]
)
@login_required
@teacher_required
def edit_evaluation(evaluation_id):
    ...
    if request.method == "POST":
        try:
            evaluation.evaluation = request.form.get(
                "evaluation", evaluation.evaluation
            )
            ...
            return redirect(
                url_for(
                    "teacher_class_management.class_details",
