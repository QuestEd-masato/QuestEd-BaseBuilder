...
def dashboard():
    ...
    current_app.logger.info(
        f"[DASHBOARD] Teacher {current_user.id} ({current_user.username}): "
    ...
    for class_obj in classes:
        ...
        for curriculum in curriculums:
            ...
            if conversion_status.get("is_converted", False):
                ...
                curriculum_stats["total_units"] += conversion_status.get(
                    "converted_units", 0
                )
    ...
    if integrated_stats["total_curriculums"] > 0:
        integrated_stats["conversion_rate"] = round(
            (
                integrated_stats["converted_curriculums"]
                / integrated_stats["total_curriculums"]
            )
            * 100,
            1,
        )
    ...
    # 課題統計の取得
    task_stats = get_teacher_task_statistics(current_user.id)
