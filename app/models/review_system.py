...
class ReviewSet(db.Model):
    ...
    generation_type = db.Column(db.Enum('automatic', 'manual', 'ai_generated'), default='automatic', comment='生成タイプ')
    ...
    review_type = db.Column(db.Enum('spaced_repetition', 'weakness_focused', 'comprehensive', 'exam_prep'), default='weakness_focused', comment='復習タイプ')
    status = db.Column(db.Enum('draft', 'active', 'completed', 'expired'), default='draft', comment='ステータス')
...
class ReviewSetItem(db.Model):
    ...
    review_set_id = db.Column(db.Integer, db.ForeignKey('review_sets.id'), nullable=False, comment='復習セットID')
    problem_id = db.Column(db.Integer, db.ForeignKey('basic_knowledge_items.id'), nullable=False, comment='問題ID')
    ...
    # ユニーク制約とインデックス
    __table_args__ = (
        db.UniqueConstraint('review_set_id', 'problem_id', name='uk_review_set_problem'),
    ...
    def to_dict(self):
        ...
        return {
            ...
            'expected_difficulty': float(self.expected_difficulty) if self.expected_difficulty else None,
