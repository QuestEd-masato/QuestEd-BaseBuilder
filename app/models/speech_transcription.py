...
class SpeechTranscription(db.Model):
    ...
    user_id = db.Column(db.Integer, db.ForeignKey('users.id'), nullable=False, comment='ユーザーID (RDS互換)')
    ...
    confidence_score = db.Column(db.Numeric(3,2), default=0.00, comment='認識精度スコア (新機能)')
    ...
    updated_at = db.Column(db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow, comment='更新日時 (新機能)')
