...
class CurriculumUnit(db.Model):
    ...
    subject_id = db.Column(
        db.Integer, db.ForeignKey("subjects.id"), nullable=True, comment="教科ID"
    )
    ...
    estimated_minutes = db.Column(db.Integer, default=50, comment='推定学習時間（分） - 50分/コマ基準')
    estimated_classes = db.Column(db.Float, default=1.0, comment='推定コマ数（50分/コマ基準）')
    ...
    school_id = db.Column(
        ...
        nullable=True,
        comment="学校ID（NULL=全校共通）",
    )
    created_by = db.Column(
        db.Integer, db.ForeignKey("users.id"), nullable=False, comment="作成者ID"
    )
    legacy_curriculum_id = db.Column(
        db.Integer,
        db.ForeignKey("curriculums.id"),
        nullable=True,
        comment="レガシーカリキュラムID",
    )
    ...
    creator = db.relationship(
        "User", foreign_keys=[created_by], backref="created_units"
    )
    legacy_curriculum = db.relationship("Curriculum", backref="migrated_units")

    # 単元と問題の紐付け
    unit_mappings = db.relationship(
        "UnitItemMapping",
    ...
    # 生徒の選択履歴
    student_selections = db.relationship(
    ...
    def to_dict(self):
        ...
        return {
            ...
            'estimated_classes': self.estimated_classes,
    ...
    def get_mapped_problems(self):
        ...
        mappings = (
            self.unit_mappings.filter_by(is_required=True)
            .order_by(UnitItemMapping.order_index)
            .all()
        )
        problem_ids = [mapping.item_id for mapping in mappings]
        return BasicKnowledgeItem.query.filter(
            BasicKnowledgeItem.id.in_(problem_ids)
        ).all()
    ...
    def calculate_estimated_classes(self):
        """推定コマ数を計算して更新"""
        if self.estimated_minutes:
            self.estimated_classes = round(self.estimated_minutes / 50, 1)
        else:
            self.estimated_classes = 1.0
...
class UnitItemMapping(db.Model):
    ...
    # リレーションシップ
    basic_knowledge_item = db.relationship(
        "BasicKnowledgeItem", backref="unit_mappings"
    )
...
class StudentUnitSelection(db.Model):
    ...
    class_id = db.Column(
        db.Integer, db.ForeignKey("classes.id"), nullable=True, comment="クラスID"
    )
    status = db.Column(
        db.Enum("not_started", "in_progress", "completed", "paused"),
        default="not_started",
        comment="学習状況",
    )
    ...
    __table_args__ = (
        db.UniqueConstraint(
            "student_id", "unit_id", "class_id", name="uk_student_unit_class"
        ),
    ...
    student = db.relationship(
        "User", foreign_keys=[student_id], backref="unit_selections"
    )
    ...
    def can_request_completion(self):
        """完了申請が可能かチェック"""
        return (
            self.status in ["in_progress", "completed"]
            and self.approval_status in ["none", "rejected"]
            and self.progress_percentage >= 80
        )  # 80%以上で申請可能
...
class ClassLearningSettings(db.Model):
    ...
    class_id = db.Column(
        db.Integer, db.ForeignKey("classes.id"), nullable=False, comment="クラスID"
    )
    ...
    min_completion_rate = db.Column(
        db.Numeric(5, 2), default=80.00, comment="単元完了最低正解率（%）"
    )
    ...
    auto_approve_threshold = db.Column(
        db.Numeric(5, 2), default=90.00, comment="自動承認閾値（%）"
    )
    ...
    created_by = db.Column(
        db.Integer, db.ForeignKey("users.id"), nullable=False, comment="設定作成者ID"
    )
    created_at = db.Column(db.DateTime, default=datetime.utcnow, comment="作成日時")
    updated_at = db.Column(
        db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow, comment="更新日時"
    ...
    class_obj = db.relationship(
        "Class", backref=db.backref("learning_settings", uselist=False)
    )
    creator = db.relationship(
        "User", foreign_keys=[created_by], backref="created_learning_settings"
    )
    ...
    def to_dict(self):
        ...
        return {
            ...
            "min_completion_rate": float(self.min_completion_rate)
            if self.min_completion_rate
            else None,
...
class LearningPath(db.Model):
    ...
    creator = db.relationship(
        "User", foreign_keys=[created_by], backref="created_paths"
    )
