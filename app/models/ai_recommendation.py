...
class AIRecommendation(db.Model):
    ...
    recommendation_type = db.Column(
        db.Enum("unit", "problem", "study_path", "review", "challenge"),
        nullable=False,
        comment="推薦タイプ",
    )
    ...
    confidence_score = db.Column(
        db.Numeric(3, 2), default=0.00, comment="AI推薦信頼度（0.00-1.00）"
    )
    ...
    student = db.relationship(
        "User", backref=db.backref("ai_recommendations", cascade="all, delete-orphan")
    )
