...
class EmailLog(db.Model):
    ...
    recipient_id = db.Column(db.Integer, db.ForeignKey('users.id'), nullable=True, comment='受信者ID')
    recipient_email = db.Column(db.String(100), nullable=False, comment='受信者メールアドレス')
    sender_email = db.Column(db.String(100), default='noreply@quested.com', comment='送信者メールアドレス')
    email_type = db.Column(db.Enum('verification', 'reset_password', 'notification', 'reminder', 'report'), nullable=False, comment='メール種類')
    ...
    status = db.Column(db.Enum('pending', 'sent', 'failed', 'bounced'), default='pending', comment='送信状況')
    ...
    def is_deliverable(self):
        """再送信可能かチェック"""
        return self.status in ['pending', 'failed'] and self.retry_count < self.max_retries
    
    def get_delivery_rate(self):
        """配信率を計算（同一受信者の統計）"""
        total_emails = EmailLog.query.filter_by(recipient_email=self.recipient_email).count()
