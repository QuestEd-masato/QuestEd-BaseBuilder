...
class DataAccessLayer(BaseService):
    ...
    def safe_query(self, model_class, filters: Optional[Dict] = None, 
        ...
        try:
            ...
            result = query.all()
            self.log_info(f"Query executed: {model_class.__name__}, results: {len(result)}")
    ...
    def safe_get_by_id(self, model_class, record_id: int):
        ...
        try:
            ...
            self.log_info(f"Get by ID: {model_class.__name__}({record_id}) - {'Found' if result else 'Not found'}")
            return result
    ...
    def clear_cache(self, pattern: Optional[str] = None):
        ...
        if pattern:
            ...
            self.log_info(f"Cache cleared: pattern '{pattern}', {len(keys_to_delete)} items")
        else:
