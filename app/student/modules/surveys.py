...
from app.models import db, InterestSurvey, PersonalitySurvey
from ..utils import student_required, get_student_survey_status
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            try:
                if existing_survey:
                    # 既存のアンケートを更新
                    existing_survey.interests = json.dumps(survey_data, ensure_ascii=False)
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            try:
                if existing_survey:
                    # 既存のアンケートを更新
                    existing_survey.personality = json.dumps(survey_data, ensure_ascii=False)
                    existing_survey.updated_at = datetime.utcnow()
...
def api_survey_status():
    ...
    try:
        ...
        detailed_status = {
            ...
            'interest_updated_at': interest_survey.updated_at.isoformat() if interest_survey and interest_survey.updated_at else None,
            'personality_updated_at': personality_survey.updated_at.isoformat() if personality_survey and personality_survey.updated_at else None,
