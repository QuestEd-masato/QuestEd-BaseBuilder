...
from app.models import Class, ClassEnrollment, CurriculumUnit, MainTheme, Milestone, db
...
def class_detail(class_id):
    ...
    try:
        ...
        enrollment = ClassEnrollment.query.filter_by(
            student_id=current_user.id, class_id=class_id, is_active=True
        ).first()
        ...
        upcoming_milestones = [
            m for m in milestones if m.due_date and m.due_date >= today
        ]
        ...
        try:
            curriculum_units = (
                ...
                .order_by(CurriculumUnit.subject, CurriculumUnit.order)
                .all()
            )
        except:
            curriculum_units = []
        ...
        for subject, units in curriculum_by_subject.items():
            curriculum_data.append(
                {
                    "curriculum": {"title": subject, "description": f"{subject}の学習単元"},
                    "items": units,
                }
            )
...
def milestone_detail(milestone_id):
    ...
    try:
        ...
        # 学生がこのクラスに所属しているかチェック
        enrollment = ClassEnrollment.query.filter_by(
            student_id=current_user.id, class_id=milestone.class_id, is_active=True
        ).first()
        ...
        related_activities = (
            ActivityRecord.query.filter_by(
                student_id=current_user.id, class_id=milestone.class_id
            )
            .filter(ActivityRecord.created_at >= milestone.created_at)
