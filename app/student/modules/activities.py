...
from app.models import ActivityLog, Class, ClassEnrollment, InquiryTheme, db

from ..utils import (
    allowed_file,
    check_class_access,
    format_activity_content,
    get_upload_path,
    ...
    try:
        ...
        if selected_class:
            ...
            main_theme = MainTheme.query.filter_by(
                class_id=selected_class.id, is_active=True
            ).first()
    ...
    try:
        ...
        if request.method == "POST":
                ...
                if allowed_file(image_file.filename) and validate_file_size(image_file):
                    ...
                    if validate_image(image_file.stream):
                        ...
                        if filename:
                            ...
                            new_activity.image_url = url_for(
            ...
            try:
                ...
                return redirect(
                    url_for("student_activities.activities", class_id=class_id)
                )
    ...
    try:
        ...
        if request.method == "POST":
            ...
            if not content:
                flash("活動内容を入力してください。", "error")
                return render_template(
                    "edit_activity.html", activity=activity, classes=classes
                )
            ...
            try:
                ...
                return redirect(
                    url_for("student_activities.view_activity", activity_id=activity_id)
                )
        ...
        return render_template("edit_activity.html", activity=activity, classes=classes)
...
def _export_activities_pdf(activities, filename_suffix):
    ...
    try:
        ...
        content_style = ParagraphStyle(
            "CustomContent", parent=styles["Normal"], fontSize=10, spaceAfter=12
        )

        # タイトル
        story.append(
            Paragraph(f"{current_user.username}の活動記録{filename_suffix}", title_style)
        )
        ...
        for activity in activities:
            ...
            if activity.theme:
                story.append(
                    Paragraph(f"テーマ: {activity.theme.title}", styles["Normal"])
                )
