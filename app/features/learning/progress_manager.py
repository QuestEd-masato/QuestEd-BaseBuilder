...
class LearningProgressManager(BaseService):
    ...
    def get_student_progress(
        self, student_id: int, unit_id: Optional[int] = None
    ) -> Dict[str, Any]:
        """
        学生の学習進捗を取得
        ...
        try:
            ...
            progress_data = {
                ...
                "completed_units": len(
                    [s for s in selections if s.status == "completed"]
                ),
                "in_progress_units": len(
                    [s for s in selections if s.status == "in_progress"]
                ),
                "units": [],
            }
            ...
            if progress_data["total_units"] > 0:
                progress_data["completion_rate"] = (
                    progress_data["completed_units"] / progress_data["total_units"]
                )
            else:
    ...
    def update_unit_progress(
        ...
        try:
            ...
            if success:
                self.log_info(
                    f"Progress updated: student {student_id}, unit {unit_id}, progress {progress_percentage}%"
                )
    ...
    def request_completion(
        self, student_id: int, unit_id: int, comment: str = ""
    ) -> Dict[str, Any]:
        """
        単元完了申請
        ...
        try:
            ...
            if success:
                ...
                self.log_info(
                    f"Completion requested: student {student_id}, unit {unit_id}"
                )
            else:
    ...
    def approve_completion(
        self, selection_id: int, teacher_id: int, comment: str = ""
    ) -> Dict[str, Any]:
        """
        完了申請の承認
        ...
        try:
            ...
            if success:
                ...
                self.log_info(
                    f"Completion approved: selection {selection_id}, teacher {teacher_id}"
                )
            else:
    ...
    def _update_completed_items(
        self, student_id: int, unit_id: int, completed_items: List[int]
    ):
        """完了アイテムの更新"""
