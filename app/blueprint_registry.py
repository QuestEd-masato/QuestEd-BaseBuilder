...
class BlueprintRegistry:
    ...
    def register_blueprint(self, app: Flask, blueprint: Blueprint, 
                         config: BlueprintConfig) -> bool:
        """
        Blueprintの登録
        ...
        try:
            ...
            if not config.enabled:
                self.logger.info(f"Blueprint {config.name} is disabled, skipping registration")
                return False
            ...
            self.logger.info(f"Blueprint {config.name} registered successfully at {config.url_prefix}")
            return True
    ...
    def register_multiple_blueprints(self, app: Flask, 
                                   blueprint_configs: List[tuple]) -> Dict[str, bool]:
        """
        複数Blueprintの一括登録
    ...
    def _sort_by_dependencies(self, blueprint_configs: List[tuple]) -> List[tuple]:
        ...
        return sorted(blueprint_configs, 
                     key=lambda x: len(x[1].dependencies or []))
...
class OptimizedBlueprintManager:
    ...
    def setup_application_blueprints(self, app: Flask) -> bool:
        ...
        try:
            ...
            # 結果確認
            failed_registrations = [name for name, success in results.items() if not success]
            
            if failed_registrations:
                app.logger.warning(f"Failed to register blueprints: {failed_registrations}")
                return False
